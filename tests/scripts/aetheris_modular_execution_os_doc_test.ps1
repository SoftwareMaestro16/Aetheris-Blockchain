param(
  [string]$Doc = "docs\architecture\aetheris-modular-execution-os.md",
...
Assert-Contains `
  -Text $readmeText `
  -Pattern ([regex]::Escape('docs/architecture/aetheris-modular-execution-os.md')) `
  -Message "README must link to modular execution OS doc"

Assert-Contains `
  -Text $shardingText `
  -Pattern ([regex]::Escape('aetheris-modular-execution-os.md')) `
  -Message "sharding R&D doc must cross-link modular execution OS doc"

Write-Host "aetheris modular execution OS doc test passed"
