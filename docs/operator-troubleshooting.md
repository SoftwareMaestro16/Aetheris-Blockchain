...
This runbook is for local Orbitalis prototype operation. It uses localnet homes, local endpoints, and `--keyring-backend test` only. Never paste mnemonics, validator keys, node keys, or keyring files into tickets, chats, logs, or diagnostic bundles.
...
| --- | --- | --- | --- |
| Binary missing | `build\orbitalisd.exe` is not found; scripts fail before init/start. | `Test-Path build\orbitalisd.exe`; `.\scripts\build-orbitalisd.ps1`; `build\orbitalisd.exe version --long --output json` | Rebuild with `.\scripts\build-orbitalisd.ps1`. If build fails, run `go test ./...` and check the build output before localnet work. |
