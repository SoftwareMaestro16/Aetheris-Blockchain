...
| Unauthorized tokenfactory action | Non-admin mint/burn/change-admin and burn from another account are rejected; native `norb`/`ORB` spoofing is rejected. | `x/tokenfactory/keeper/msg_server_test.go` |
...
- Signed CLI tokenfactory burn/change-admin is not yet in the one-command acceptance suite.
