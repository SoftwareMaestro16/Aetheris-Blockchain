...
`x/events` atomic tasks are expanded into task-specific records for signature
validation, replay prevention, sequence integrity, fee/priority abuse, bank
sends, multi-send atomicity, supply consistency, zero-address handling,
...
spoofing prevention, event receipt linkage, and event-as-authority rejection.
These records must not collapse back to generic module-level attack
descriptions.
