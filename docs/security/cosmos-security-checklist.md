...
- Module accounts are registered with the minimum permissions required for tokenfactory, fees, staking, bank, mint, and distribution interactions.
...
- Native `norb`, staking, and fees denoms cannot be spoofed or overwritten.
...
- `go mod verify`, dependency advisories, CodeQL, and dependency scan findings are either clean or triaged with owner and milestone.
...
| Missing denom validation | tokenfactory, fees | `app/app_test.go`, `x/tokenfactory/keeper/msg_server_test.go`, `x/fees/keeper/ante_test.go` |
| Insufficient authorization | tokenfactory, fees params | `x/tokenfactory/keeper/msg_server_test.go`, `x/fees/keeper/msg_server_test.go` |
| Missing bank balance or accounting check | tokenfactory, fees accounting | `x/tokenfactory/keeper/msg_server_test.go`, `tests/e2e/tokenfactory_smoke.ps1` |
...
| Malformed query request | fees, tokenfactory, bank/staking examples | `x/*/keeper/query_server_test.go`, `tests/e2e/query_surface_smoke.ps1` |
