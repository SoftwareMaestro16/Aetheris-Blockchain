...
| `CreatePool` | DEX | User sends both pool coins to `dex`; `dex` mints LP; `dex` sends LP to creator. | Creator Bech32, canonical distinct positive coins, duplicate pair check, positive initial shares. | Pool, pair index, and next id write after all bank movements in cached block. | All bank/store errors checked; cached block prevents reserve custody or LP supply partial writes. | create-pool tests, duplicate pair tests, accounting invariant test. |
...
| Ante fee validation | fees | No custom bank movement; SDK ante handles fee deduction. | FeeTx shape, non-empty positive fee, single allowed denom `norb`, params validation. | No fees module state write in ante. | Wrong/malformed fee fails before tx execution. | fees ante unit and e2e tests. |
