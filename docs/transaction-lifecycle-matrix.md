...
| Staking delegate | local delegator bonds `naet` to a bonded validator | `--from node0`; SDK `MsgDelegate` delegator signer | `tx staking delegate $VALOPER 5000000naet --fees 1000000naet` | delegator must hold delegated `naet` plus fee; bond denom must be `naet` | `x/staking` delegation, shares, validator token/power state; bank bonded pool movement | SDK tx/message, staking delegation, and coin movement events | `query staking delegation $DELEGATOR $VALOPER`; `query staking validator $VALOPER`; CometBFT validator set/voting power | `app/pos_test.go`, `tests/e2e/pos_smoke.ps1`, `tests/e2e/prototype_acceptance.ps1` |
...
| Staking delegate | SDK requires delegator signature | malformed `aevaloper`; wrong delegation denom | insufficient funds and invalid validator fail safely | `app/pos_test.go` and `tests/e2e/pos_smoke.ps1` cover signed replay/sequence | staking keeper direct validator/delegation keys |
