...
| DEX create pool | liquidity provider creates a constant-product pair | `--from node0`; `MsgCreatePool.Creator` from CLI account | `tx dex create-pool 10000000norb "10000000$DENOM" --fees 1000000norb` | creator must hold both pool coins plus `norb` fee | user coins move to `dex` module account; LP `lp/<pool_id>` minted to creator; pool, pair index, next pool id written | `dex_create_pool` plus SDK tx/message, bank transfer/mint events | `query dex pool 1`; `query bank balance $CREATOR lp/1`; module account balance checks in tests | `x/dex/keeper/msg_server_test.go`, `x/dex/keeper/query_server_test.go`, `tests/e2e/dex_smoke.ps1`, `tests/e2e/prototype_acceptance.ps1` |
