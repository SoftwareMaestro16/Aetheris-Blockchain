...
The smoke uses `.localnet-mempool-negative` and shifted ports by default, creates one tokenfactory denom, then submits invalid transactions over normal CLI/broadcast paths.
...
- Rejection paths rely on bounded parsing, direct key lookups, and cached state changes in tokenfactory handlers.
