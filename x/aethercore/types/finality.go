...
func NewFinalityRecord(snapshot RootSnapshot, appHash string, commitmentCount uint64, routingTableRoot string, crossZoneDelay uint64) (FinalityRecord, error) {
	...
	if err := ValidateHash("aethercore finality app hash", appHash); err != nil {
		return FinalityRecord{}, err
	}
	if routingTableRoot != "" {
		if err := ValidateHash("aethercore finality routing table root", routingTableRoot); err != nil {
			return FinalityRecord{}, err
		}
	}
	if crossZoneDelay == 0 {
...
func (r FinalityRecord) ValidateFormat() error {
	if r.Height == 0 {
		return errors.New("aethercore finality record height must be positive")
	}
	for _, field := range []struct {
		name  string
		value string
	}{
		{"aethercore finality app hash", r.AppHash},
		{"aethercore finality global state root", r.GlobalStateRoot},
		{"aethercore finality global message root", r.GlobalMessageRoot},
		{"aethercore finality execution receipt root", r.ExecutionReceiptRoot},
		{"aethercore finality zone commitments root", r.ZoneCommitmentsRoot},
	} {
	...
	if r.RoutingTableRoot != "" {
		if err := ValidateHash("aethercore finality routing table root", r.RoutingTableRoot); err != nil {
			return err
		}
	}
	if r.CommitmentCount == 0 {
		return errors.New("aethercore finality commitment count must be positive")
	}
	if r.CrossZoneFinalityDelay == 0 {
		return errors.New("aethercore finality delay must be positive")
	}
	if r.EligibleDeliveryHeight <= r.Height {
		return errors.New("aethercore finality eligible delivery height must follow height")
	}
	if r.EligibleDeliveryHeight != r.Height+r.CrossZoneFinalityDelay {
		return errors.New("aethercore finality eligible delivery height mismatch")
	}
	if r.FinalityHash != "" {
		return ValidateHash("aethercore finality record hash", r.FinalityHash)
	}
	return nil
}
...
func (r FinalityRecord) ValidateHash() error {
	...
	if r.FinalityHash != expected {
		return fmt.Errorf("aethercore finality record hash mismatch: expected %s", expected)
	}
	return nil
}
...
func ComputeFinalityRecordHash(record FinalityRecord) string {
	return hashParts(
		"aetheris-aek-finality-record-v1",
...
func computeSnapshotZoneCommitmentsRoot(snapshot RootSnapshot) string {
	...
	parts := []string{"aetheris-aek-finality-zone-commitments-v1", fmt.Sprint(snapshot.Height), fmt.Sprint(len(snapshot.ProofRoots))}
	for _, root := range snapshot.ProofRoots {
