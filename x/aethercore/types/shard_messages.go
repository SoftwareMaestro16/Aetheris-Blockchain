...
func NewShardMessageEnvelope(msg ShardMessageEnvelope) (ShardMessageEnvelope, error) {
	if msg.MessageHash != "" {
		return ShardMessageEnvelope{}, errors.New("aethercore shard message hash must be empty before construction")
	}
...
func (m ShardMessageEnvelope) ValidateHash() error {
	...
	if m.MessageHash != ComputeShardMessageHash(m) {
		return errors.New("aethercore shard message hash mismatch")
	}
	return nil
}

func (m ShardMessageEnvelope) ValidateFormat() error {
	if err := validateToken("aethercore shard message id", m.MsgID, MaxScopeLength); err != nil {
		return err
	}
	if err := validateToken("aethercore shard message trace id", m.TraceID, MaxScopeLength); err != nil {
		return err
	}
	if err := validateToken("aethercore shard message sender", m.Sender, MaxScopeLength); err != nil {
		return err
	}
	if err := validateToken("aethercore shard message receiver", m.Receiver, MaxScopeLength); err != nil {
		return err
	}
	...
	if err := validateToken("aethercore shard message destination key", m.DestinationStateKey, MaxScopeLength); err != nil {
		return err
	}
	if err := validateToken("aethercore shard message payload type", m.PayloadType, MaxScopeLength); err != nil {
		return err
	}
	if err := ValidateHash("aethercore shard message payload hash", m.PayloadHash); err != nil {
		return err
	}
	if m.AdmissionHeight == 0 || m.CreatedAtHeight == 0 || m.ExpiryHeight == 0 {
		return errors.New("aethercore shard message heights must be positive")
	}
	if m.ExpiryHeight <= m.CreatedAtHeight {
		return errors.New("aethercore shard message expiry height must be future")
	}
	if m.SourceLayoutEpoch == 0 || m.DeliveryLayoutEpoch == 0 {
		return errors.New("aethercore shard message layout epochs must be positive")
	}
	if m.MessageHash != "" {
		return ValidateHash("aethercore shard message hash", m.MessageHash)
	}
	return nil
}
...
func NewShardMessageStore(store ShardMessageStore) (ShardMessageStore, error) {
	if store.StoreRoot != "" {
		return ShardMessageStore{}, errors.New("aethercore shard message store root must be empty before construction")
	}
	normalized := store
...
func (s ShardMessageStore) ValidateHash() error {
	...
	if s.StoreRoot != ComputeShardMessageStoreRoot(s) {
		return errors.New("aethercore shard message store root mismatch")
	}
	return nil
}
...
func (s ShardMessageStore) ValidateFormat() error {
	...
	if s.Height == 0 {
		return errors.New("aethercore shard message store height must be positive")
	}
	if !IsShardQueueKind(s.QueueKind) {
		return fmt.Errorf("unknown aethercore shard queue kind %q", s.QueueKind)
	}
	for i, msg := range s.Messages {
		...
		if i > 0 && compareShardMessages(s.Messages[i-1], msg) >= 0 {
			return errors.New("aethercore shard messages must be sorted canonically")
		}
	}
	if s.StoreRoot != "" {
		return ValidateHash("aethercore shard message store root", s.StoreRoot)
	}
	return nil
}
...
func BuildShardMessageStoreEntries(store ShardMessageStore) ([]ShardMessageStoreEntry, error) {
	...
	for i, msg := range store.Messages {
		...
		default:
			err = fmt.Errorf("unknown aethercore shard queue kind %q", store.QueueKind)
		}
		if err != nil {
			return nil, err
		}
...
func ResolveShardMessageDeliveryRoute(layouts []ShardLayout, msg ShardMessageEnvelope) (ShardRoute, error) {
	...
	if !found {
		return ShardRoute{}, errors.New("aethercore shard message delivery missing committed layout")
	}
...
func ExecuteShardMigrationTasks(tasks []ShardMigrationTask, height uint64, stateRootBefore string) ([]ShardMigrationReceipt, string, error) {
	if height == 0 {
		return nil, "", errors.New("aethercore shard migration execution height must be positive")
	}
	if err := ValidateHash("aethercore shard migration state root before", stateRootBefore); err != nil {
		return nil, "", err
	}
	...
	for i, task := range ordered {
		...
		receipt.StateRootAfter = hashParts("aetheris-aek-shard-migration-state-root-v1", currentRoot, task.TaskHash, fmt.Sprint(height))
...
func (r ShardMigrationReceipt) ValidateHash() error {
	...
	if r.ReceiptHash != ComputeShardMigrationReceiptHash(r) {
		return errors.New("aethercore shard migration receipt hash mismatch")
	}
	return nil
}

func (r ShardMigrationReceipt) ValidateFormat() error {
	if err := validateToken("aethercore shard migration receipt task id", r.TaskID, MaxIDLength); err != nil {
		return err
	}
	if err := ValidateHash("aethercore shard migration receipt task hash", r.TaskHash); err != nil {
		return err
	}
	...
	if r.SourceLayoutEpoch == 0 || r.TargetLayoutEpoch == 0 || r.DeliveryEpoch == 0 {
		return errors.New("aethercore shard migration receipt epochs must be positive")
	}
	if r.TargetLayoutEpoch <= r.SourceLayoutEpoch {
		return errors.New("aethercore shard migration receipt target epoch must be future")
	}
	if r.DeliveryEpoch < r.TargetLayoutEpoch {
		return errors.New("aethercore shard migration receipt delivery epoch must not precede target epoch")
	}
	if r.Height == 0 {
		return errors.New("aethercore shard migration receipt height must be positive")
	}
	if !IsShardMigrationStatus(r.Status) {
		return fmt.Errorf("unknown aethercore shard migration status %q", r.Status)
	}
	if err := ValidateHash("aethercore shard migration receipt state root before", r.StateRootBefore); err != nil {
		return err
	}
	if err := ValidateHash("aethercore shard migration receipt state root after", r.StateRootAfter); err != nil {
		return err
	}
	if r.ReceiptHash != "" {
		return ValidateHash("aethercore shard migration receipt hash", r.ReceiptHash)
	}
	return nil
}
...
func ComputeShardMessageStoreRoot(store ShardMessageStore) string {
	...
	parts := []string{
		"aetheris-aek-shard-message-store-v1",
		string(store.ZoneID),
...
func ComputeShardMigrationReceiptHash(receipt ShardMigrationReceipt) string {
	return hashParts(
		"aetheris-aek-shard-migration-receipt-v1",
		receipt.TaskID,
		receipt.TaskHash,
...
func ComputeShardMigrationReceiptRoot(receipts []ShardMigrationReceipt) (string, error) {
	...
	parts := []string{"aetheris-aek-shard-migration-receipt-root-v1", fmt.Sprint(len(ordered))}
...
func validateShardMessageStoreRoute(store ShardMessageStore, msg ShardMessageEnvelope) error {
	...
	case ShardQueueInbox:
		if msg.ReceiverZoneID != store.ZoneID {
			return errors.New("aethercore shard inbox message zone mismatch")
		}
		if msg.ReceiverShardID != "" && msg.ReceiverShardID != store.ShardID {
			return errors.New("aethercore shard inbox message shard mismatch")
		}
	case ShardQueueOutbox:
		if msg.SenderZoneID != store.ZoneID || msg.SenderShardID != store.ShardID {
			return errors.New("aethercore shard outbox message route mismatch")
		}
	default:
		return fmt.Errorf("unknown aethercore shard queue kind %q", store.QueueKind)
	}
	return nil
}
