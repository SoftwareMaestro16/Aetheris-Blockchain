...
type AetherCoreState = CoreState

func EmptyState(params ...AetherCoreParams) CoreState {
...
func RegisterZone(state CoreState, descriptor ZoneDescriptor) (CoreState, error) {
	...
	if _, found := state.ZoneDescriptorByID(descriptor.ZoneID); found {
		return CoreState{}, fmt.Errorf("aethercore zone descriptor %s already registered", descriptor.ZoneID)
	}
	next := state.Clone()
...
func RegisterServiceDescriptor(state CoreState, descriptor ServiceDescriptor) (CoreState, error) {
	...
	if _, found := state.ZoneDescriptorByID(descriptor.ZoneID); !found {
		return CoreState{}, fmt.Errorf("aethercore service zone %s is not registered", descriptor.ZoneID)
	}
	if _, found := state.ServiceByID(descriptor.ServiceID); found {
		return CoreState{}, fmt.Errorf("aethercore service descriptor %s already registered", descriptor.ServiceID)
	}
	next := state.Clone()
...
func RegisterShardLayout(state CoreState, layout ShardLayout) (CoreState, error) {
	...
	if !found {
		return CoreState{}, fmt.Errorf("aethercore shard layout zone %s is not registered", layout.ZoneID)
	}
	if !descriptor.Enabled {
		return CoreState{}, fmt.Errorf("aethercore shard layout zone %s is disabled", layout.ZoneID)
	}
	if uint32(len(layout.ActiveShards)) > descriptor.MaxShards {
		return CoreState{}, fmt.Errorf("aethercore shard layout exceeds zone max shards %d", descriptor.MaxShards)
	}
	for _, existing := range state.ShardLayouts {
		if existing.ZoneID == layout.ZoneID && existing.LayoutEpoch == layout.LayoutEpoch {
			return CoreState{}, errors.New("duplicate aethercore shard layout epoch")
		}
	}
	next := state.Clone()
...
func CommitRoutingTable(state CoreState, table RoutingTableCommitment) (CoreState, error) {
	...
	for _, entry := range table.Entries {
		...
		if !found {
			return CoreState{}, fmt.Errorf("aethercore routing table references missing layout %s/%d", entry.ZoneID, entry.LayoutEpoch)
		}
		if layout.LayoutHash != entry.LayoutHash {
			return CoreState{}, errors.New("aethercore routing table layout hash mismatch")
		}
		if uint32(len(layout.ActiveShards)) != entry.ActiveShards {
			return CoreState{}, errors.New("aethercore routing table active shard count mismatch")
		}
		descriptor, found := state.ZoneDescriptorByID(entry.ZoneID)
		if !found {
			return CoreState{}, fmt.Errorf("aethercore routing table zone %s is not registered", entry.ZoneID)
		}
		if !descriptor.Enabled {
			return CoreState{}, fmt.Errorf("aethercore routing table zone %s is disabled", entry.ZoneID)
		}
	}
	for _, existing := range state.RoutingTables {
		if existing.RoutingEpoch == table.RoutingEpoch {
			return CoreState{}, errors.New("duplicate aethercore routing table epoch")
		}
	}
	next := state.Clone()
...
func AppendZoneCommitment(state CoreState, commitment ZoneCommitment) (CoreState, error) {
	...
	if !found {
		return CoreState{}, fmt.Errorf("aethercore zone %s is not registered", commitment.ZoneID)
	}
	if !descriptor.Enabled {
		return CoreState{}, fmt.Errorf("aethercore zone %s is disabled", commitment.ZoneID)
	}
	for _, existing := range state.ZoneCommitments {
		if existing.Height == commitment.Height && existing.ZoneID == commitment.ZoneID {
			return CoreState{}, errors.New("duplicate aethercore zone commitment height")
		}
	}
	next := state.Clone()
...
func CommitBlockRootsWithContributions(state CoreState, height uint64, contributions RootContributions) (CoreState, RootSnapshot, error) {
	...
	for i, commitment := range commitments {
		proofRoots[i] = ProofRoot{
			...
			Source:   "aethercore.zone_commitments",
		}
	}
	...
	if table, found := state.LatestRoutingTableAtHeight(height); found {
		proofRoots = append(proofRoots, ProofRoot{
			...
			Source:    "aethercore.routing_table",
...
func AppendGlobalRoot(state CoreState, root GlobalStateRoot) (CoreState, error) {
	...
	for _, existing := range state.GlobalRoots {
		if existing.Height == root.Height {
			return CoreState{}, errors.New("duplicate aethercore global root height")
		}
	}
	next := state.Clone()
...
func AddExportManifest(state CoreState, manifest ExportManifest) (CoreState, error) {
	...
	if !found {
		return CoreState{}, fmt.Errorf("aethercore global root for height %d is not found", manifest.Height)
	}
...
func (s CoreState) ShardLayoutProofRootsAtHeight(height uint64) []ProofRoot {
	...
	for _, zoneID := range zones {
		...
		roots = append(roots, ProofRoot{
			...
			Source:    "aethercore.shard_layout",
...
func (r ProofRoot) Validate() error {
	if r.Height == 0 {
		return errors.New("aethercore proof root height must be positive")
	}
	if err := validateToken("aethercore proof root type", string(r.RootType), MaxScopeLength); err != nil {
		return err
	}
	if err := ValidateHash("aethercore proof root hash", r.RootHash); err != nil {
		return err
	}
	if r.ZoneID != "" {
	...
	if r.Source != "" {
		return validateToken("aethercore proof root source", r.Source, MaxScopeLength)
	}
	return nil
}
...
func (s RootSnapshot) Validate() error {
	if s.Height == 0 {
		return errors.New("aethercore root snapshot height must be positive")
	}
	...
	if s.Finality.GlobalStateRoot != s.GlobalStateRoot.RootHash {
		return errors.New("aethercore finality global state root mismatch")
	}
	if s.Finality.GlobalMessageRoot != s.GlobalMessageRoot.RootHash {
		return errors.New("aethercore finality global message root mismatch")
	}
	if s.Finality.ExecutionReceiptRoot != s.ExecutionReceiptRoot.RootHash {
		return errors.New("aethercore finality receipt root mismatch")
	}
...
func validateZoneDescriptors(descriptors []ZoneDescriptor, params AetherCoreParams) error {
	var previous ZoneID
	...
	for i, descriptor := range descriptors {
		...
		if _, found := seen[descriptor.ZoneID]; found {
			return fmt.Errorf("duplicate aethercore zone descriptor %s", descriptor.ZoneID)
		}
...
func validateServiceDescriptors(descriptors []ServiceDescriptor, registeredZones map[ZoneID]ZoneDescriptor) error {
	...
	for i, descriptor := range descriptors {
		...
		if _, found := registeredZones[descriptor.ZoneID]; !found {
			return fmt.Errorf("aethercore service zone %s is not registered", descriptor.ZoneID)
		}
		if _, found := seen[descriptor.ServiceID]; found {
			return fmt.Errorf("duplicate aethercore service descriptor %s", descriptor.ServiceID)
		}
		seen[descriptor.ServiceID] = struct{}{}
		if i > 0 && previous >= descriptor.ServiceID {
			return errors.New("aethercore service descriptors must be sorted canonically by service id")
		}
...
func validateShardLayouts(layouts []ShardLayout, registeredZones map[ZoneID]ZoneDescriptor) error {
	...
	for i, layout := range layouts {
		...
		if !found {
			return fmt.Errorf("aethercore shard layout zone %s is not registered", layout.ZoneID)
		}
		if !descriptor.Enabled {
			return fmt.Errorf("aethercore shard layout zone %s is disabled", layout.ZoneID)
		}
		if uint32(len(layout.ActiveShards)) > descriptor.MaxShards {
			return fmt.Errorf("aethercore shard layout exceeds zone max shards %d", descriptor.MaxShards)
		}
		key := fmt.Sprintf("%s/%020d", layout.ZoneID, layout.LayoutEpoch)
		if _, found := seen[key]; found {
			return errors.New("duplicate aethercore shard layout epoch")
		}
		seen[key] = struct{}{}
		if i > 0 && compareShardLayouts(previous, layout) >= 0 {
			return errors.New("aethercore shard layouts must be sorted canonically")
		}
		previous = layout
	}
	return nil
...
func validateRoutingTables(tables []RoutingTableCommitment, layouts []ShardLayout, registeredZones map[ZoneID]ZoneDescriptor) error {
	...
	for i, table := range tables {
		...
		if _, found := seen[table.RoutingEpoch]; found {
			return errors.New("duplicate aethercore routing table epoch")
		}
		seen[table.RoutingEpoch] = struct{}{}
		if i > 0 && previous >= table.RoutingEpoch {
			return errors.New("aethercore routing tables must be sorted canonically by epoch")
		}
		for _, entry := range table.Entries {
			descriptor, found := registeredZones[entry.ZoneID]
			if !found {
				return fmt.Errorf("aethercore routing table zone %s is not registered", entry.ZoneID)
			}
			if !descriptor.Enabled {
				return fmt.Errorf("aethercore routing table zone %s is disabled", entry.ZoneID)
			}
			layout, found := shardLayoutByEpoch(layouts, entry.ZoneID, entry.LayoutEpoch)
			if !found {
				return fmt.Errorf("aethercore routing table references missing layout %s/%d", entry.ZoneID, entry.LayoutEpoch)
			}
			if layout.LayoutHash != entry.LayoutHash {
				return errors.New("aethercore routing table layout hash mismatch")
			}
			if uint32(len(layout.ActiveShards)) != entry.ActiveShards {
				return errors.New("aethercore routing table active shard count mismatch")
			}
		}
...
func validateZoneCommitments(commitments []ZoneCommitment, registeredZones map[ZoneID]ZoneDescriptor) error {
	...
	for i, commitment := range commitments {
		...
		if !found {
			return fmt.Errorf("aethercore zone %s is not registered", commitment.ZoneID)
		}
		if !descriptor.Enabled {
			return fmt.Errorf("aethercore zone %s is disabled", commitment.ZoneID)
		}
		key := fmt.Sprintf("%020d/%s", commitment.Height, commitment.ZoneID)
		if _, found := seen[key]; found {
			return errors.New("duplicate aethercore zone commitment height")
		}
		seen[key] = struct{}{}
...
func validateGlobalRoots(roots []GlobalStateRoot) error {
	...
	for i, root := range roots {
		...
		if _, found := seen[root.Height]; found {
			return errors.New("duplicate aethercore global root height")
		}
		seen[root.Height] = struct{}{}
		if i > 0 && previous >= root.Height {
			return errors.New("aethercore global roots must be sorted canonically by height")
		}
...
func validateRootSnapshots(snapshots []RootSnapshot) error {
	...
	for i, snapshot := range snapshots {
		...
		if _, found := seen[snapshot.Height]; found {
			return errors.New("duplicate aethercore root snapshot height")
		}
		seen[snapshot.Height] = struct{}{}
		if i > 0 && previous >= snapshot.Height {
			return errors.New("aethercore root snapshots must be sorted canonically by height")
		}
...
func validateExportManifests(manifests []ExportManifest, roots []GlobalStateRoot) error {
	...
	for i, manifest := range manifests {
		...
		if _, found := seen[manifest.Height]; found {
			return errors.New("duplicate aethercore export manifest height")
		}
		seen[manifest.Height] = struct{}{}
		if i > 0 && previous >= manifest.Height {
			return errors.New("aethercore export manifests must be sorted canonically by height")
		}
		root, found := globalRootByHeight(roots, manifest.Height)
		if !found {
			return fmt.Errorf("aethercore global root for height %d is not found", manifest.Height)
		}
		if manifest.GlobalRoot != root.GlobalRoot {
			return errors.New("aethercore export manifest global root mismatch")
		}
