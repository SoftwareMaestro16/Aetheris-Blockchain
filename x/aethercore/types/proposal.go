...
func BuildProposalSchedule(height uint64, items []ProposalItem, params AetherCoreParams) (ProposalSchedule, error) {
	if height == 0 {
		return ProposalSchedule{}, errors.New("aethercore proposal schedule height must be positive")
	}
	if err := params.Validate(); err != nil {
		return ProposalSchedule{}, err
	}
	if uint32(len(items)) > params.MaxProposalItemsPerBlock {
		return ProposalSchedule{}, errors.New("aethercore proposal item count exceeds block limit")
	}
...
func ValidateProposalScheduleForState(schedule ProposalSchedule, state CoreState) error {
	...
	for _, group := range schedule.Groups {
		...
		if !found {
			return errors.New("aethercore proposal group zone is not registered")
		}
		if !descriptor.Enabled {
			return errors.New("aethercore proposal group zone is disabled")
		}
		if group.ZoneID == ZoneIDAetherCore {
			continue
		}
		layout, found := state.LatestShardLayout(group.ZoneID, schedule.Height)
		if !found {
			return errors.New("aethercore proposal group missing shard layout")
		}
		if !layout.HasActiveShard(group.ShardID) {
			return errors.New("aethercore proposal group shard is not active in committed layout")
		}
	}
	return nil
}
...
func (i ProposalItem) Validate() error {
	...
	return ValidateHash("aethercore proposal tx hash", i.TxHash)
}
...
func (g ProposalGroup) Validate() error {
	...
	for i, item := range g.Items {
		...
		if item.ZoneID != g.ZoneID || item.ShardID != g.ShardID {
			return errors.New("aethercore proposal group item route mismatch")
		}
		if i > 0 && compareProposalItems(g.Items[i-1], item) >= 0 {
			return errors.New("aethercore proposal group items must be sorted canonically")
		}
	}
	return nil
}
...
func (s ProposalSchedule) Validate() error {
	if s.Height == 0 {
		return errors.New("aethercore proposal schedule height must be positive")
	}
	for i, group := range s.Groups {
		...
		if i > 0 && compareProposalGroupKey(s.Groups[i-1], group) >= 0 {
			return errors.New("aethercore proposal groups must be sorted canonically")
		}
	}
	return nil
}
