...
func (spec CosmosModuleBoundarySpec) ValidateFormat() error {
	...
	if spec.Version != CosmosModuleBoundarySpecVersion {
		return fmt.Errorf("aethercore cosmos module boundary spec version must be %d", CosmosModuleBoundarySpecVersion)
	}
	if len(spec.Modifications) == 0 {
		return errors.New("aethercore cosmos module boundary spec requires modifications")
	}
	if len(spec.BoundaryRules) == 0 {
		return errors.New("aethercore cosmos module boundary spec requires rules")
	}
	...
	for _, modification := range spec.Modifications {
		...
		if _, found := seenModules[modification.Module]; found {
			return fmt.Errorf("aethercore duplicate existing cosmos module %s", modification.Module)
		}
		seenModules[modification.Module] = struct{}{}
		if previousModule != "" && previousModule >= modification.Module {
			return errors.New("aethercore existing cosmos module modifications must be sorted")
		}
	...
	for _, rule := range spec.BoundaryRules {
		...
		if _, found := seenRules[rule.Rule]; found {
			return fmt.Errorf("aethercore duplicate cosmos module boundary rule %s", rule.Rule)
		}
		seenRules[rule.Rule] = struct{}{}
		if previousRule != "" && previousRule >= rule.Rule {
			return errors.New("aethercore cosmos module boundary rules must be sorted")
		}
		previousRule = rule.Rule
	}
	if spec.Root != "" {
		if err := ValidateHash("aethercore cosmos module boundary spec root", spec.Root); err != nil {
			return err
		}
	}
	return nil
...
func (spec CosmosModuleBoundarySpec) Validate() error {
	...
	if spec.Root == "" {
		return errors.New("aethercore cosmos module boundary spec root is required")
	}
	expected := ComputeCosmosModuleBoundarySpecRoot(spec.Modifications, spec.BoundaryRules)
	if spec.Root != expected {
		return fmt.Errorf("aethercore cosmos module boundary spec root mismatch: expected %s", expected)
	}
	return nil
}
...
func BuildExistingCosmosModuleModification(modification ExistingCosmosModuleModification) (ExistingCosmosModuleModification, error) {
	...
	if modification.DescriptorHash != "" {
		return ExistingCosmosModuleModification{}, errors.New("aethercore existing cosmos module descriptor hash must be empty before construction")
	}
...
func (modification ExistingCosmosModuleModification) ValidateFormat() error {
	modification = modification.Normalize()
	if err := validateToken("aethercore existing cosmos module name", modification.Module, MaxScopeLength); err != nil {
		return err
	}
	if modification.RequiredModification == "" {
		return errors.New("aethercore existing cosmos module modification is required")
	}
	if modification.Boundary == "" {
		return errors.New("aethercore existing cosmos module boundary is required")
	}
	if modification.DescriptorHash != "" {
		if err := ValidateHash("aethercore existing cosmos module descriptor hash", modification.DescriptorHash); err != nil {
			return err
		}
	}
	return nil
...
func (modification ExistingCosmosModuleModification) Validate() error {
	...
	if modification.DescriptorHash == "" {
		return errors.New("aethercore existing cosmos module descriptor hash is required")
	}
	expected := ComputeExistingCosmosModuleModificationHash(modification)
	if modification.DescriptorHash != expected {
		return fmt.Errorf("aethercore existing cosmos module descriptor hash mismatch: expected %s", expected)
	}
	return nil
}
...
func BuildCosmosModuleBoundaryRule(rule CosmosModuleBoundaryRule) (CosmosModuleBoundaryRule, error) {
	...
	if rule.DescriptorHash != "" {
		return CosmosModuleBoundaryRule{}, errors.New("aethercore cosmos module boundary rule hash must be empty before construction")
	}
...
func (rule CosmosModuleBoundaryRule) ValidateFormat() error {
	rule = rule.Normalize()
	if rule.Rule == "" {
		return errors.New("aethercore cosmos module boundary rule is required")
	}
	if rule.Enforcement == "" {
		return errors.New("aethercore cosmos module boundary enforcement is required")
	}
	if rule.DescriptorHash != "" {
		if err := ValidateHash("aethercore cosmos module boundary rule hash", rule.DescriptorHash); err != nil {
			return err
		}
	}
	return nil
...
func (rule CosmosModuleBoundaryRule) Validate() error {
	...
	if rule.DescriptorHash == "" {
		return errors.New("aethercore cosmos module boundary rule hash is required")
	}
	expected := ComputeCosmosModuleBoundaryRuleHash(rule)
	if rule.DescriptorHash != expected {
		return fmt.Errorf("aethercore cosmos module boundary rule hash mismatch: expected %s", expected)
	}
	return nil
}
...
func ComputeExistingCosmosModuleModificationHash(modification ExistingCosmosModuleModification) string {
	modification = modification.Normalize()
	return hashParts(
		"aetheris-existing-cosmos-module-modification-v1",
		modification.Module,
...
func ComputeCosmosModuleBoundaryRuleHash(rule CosmosModuleBoundaryRule) string {
	rule = rule.Normalize()
	return hashParts(
		"aetheris-cosmos-module-boundary-rule-v1",
		rule.Rule,
		rule.Enforcement,
	)
}
...
func ComputeCosmosModuleBoundarySpecRoot(modifications []ExistingCosmosModuleModification, rules []CosmosModuleBoundaryRule) string {
	...
	parts := []string{"aetheris-cosmos-module-boundary-spec-v1", fmt.Sprintf("%020d", CosmosModuleBoundarySpecVersion)}
...
func ValidateCosmosModuleBoundarySpec() error {
	...
	for _, module := range requiredModules {
		if _, found := byModule[module]; !found {
			return fmt.Errorf("aethercore cosmos module boundary spec missing %s", module)
		}
	}
	...
	for _, fragment := range requiredRuleFragments {
		...
		if !found {
			return fmt.Errorf("aethercore cosmos module boundary spec missing rule %s", fragment)
		}
	}
	return nil
}
