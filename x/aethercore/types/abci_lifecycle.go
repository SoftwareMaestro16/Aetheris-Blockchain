...
func ProcessKernelABCIProposal(ctx KernelConsensusContext, state CoreState, proposal KernelABCIProposal, envelopes []KernelMessageEnvelope, limits KernelGasLimits) error {
	...
	if expected := ComputeKernelEnvelopeRoot(envelopes); expected != proposal.EnvelopeRoot {
		return errors.New("aethercore ABCI proposal envelope root mismatch")
	}
	...
	if expected := ComputeKernelRoutedMessageRoot(routed); expected != proposal.RoutedMessageRoot {
		return errors.New("aethercore ABCI proposal routed message root mismatch")
	}
	workloads := buildKernelWorkloads(envelopes)
	if !sameKernelWorkloads(workloads, proposal.Workloads) {
		return errors.New("aethercore ABCI proposal workload grouping mismatch")
	}
	if proposal.BlockGas != sumKernelGas(envelopes) {
		return errors.New("aethercore ABCI proposal block gas mismatch")
	}
	if !sameKernelZoneGas(buildKernelZoneGas(envelopes), proposal.ZoneGas) {
		return errors.New("aethercore ABCI proposal zone gas mismatch")
	}
	if expected := ComputeKernelABCIProposalHash(proposal); expected != proposal.ProposalHash {
		return fmt.Errorf("aethercore ABCI proposal hash mismatch: expected %s", expected)
	}
	return nil
}
...
func FinalizeKernelABCIBlock(ctx KernelConsensusContext, state CoreState, proposal KernelABCIProposal, envelopes []KernelMessageEnvelope, input KernelFinalizationInput, cleanupQueue []KernelCleanupItem, cleanupLimit uint64) (CoreState, KernelFinalization, KernelCleanupResult, error) {
	if cleanupLimit == 0 {
		return CoreState{}, KernelFinalization{}, KernelCleanupResult{}, errors.New("aethercore ABCI cleanup limit must be positive")
	}
...
func CommitKernelABCIBlock(finalization KernelFinalization, appHash string) (KernelABCICommitRecord, error) {
	...
	if err := ValidateHash("aethercore ABCI app hash", appHash); err != nil {
		return KernelABCICommitRecord{}, err
	}
...
func ProcessKernelCleanupQueue(height uint64, queue []KernelCleanupItem, limit uint64) (KernelCleanupResult, error) {
	if height == 0 {
		return KernelCleanupResult{}, errors.New("aethercore cleanup height must be positive")
	}
	if limit == 0 {
		return KernelCleanupResult{}, errors.New("aethercore cleanup limit must be positive")
	}
...
func (e KernelMessageEnvelope) ValidateBasic(ctx KernelConsensusContext, state CoreState) error {
	...
	if e.Kind != KernelMessageLocalTx && e.Kind != KernelMessageRoutedInbound {
		return fmt.Errorf("unknown aethercore kernel message kind %q", e.Kind)
	}
	if err := ValidateHash("aethercore kernel envelope tx hash", e.TxHash); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore kernel envelope sender", e.Sender); err != nil {
		return err
	}
	if e.Nonce == 0 {
		return errors.New("aethercore kernel envelope nonce must be positive")
	}
	if e.GasLimit == 0 {
		return errors.New("aethercore kernel envelope gas limit must be positive")
	}
	if e.AdmissionHeight == 0 || e.AdmissionHeight > ctx.Height {
		return errors.New("aethercore kernel envelope admission height is invalid")
	}
	...
	if e.Kind == KernelMessageLocalTx {
		if e.SourceZone != e.DestinationZone || e.SourceShard != e.DestinationShard {
			return errors.New("aethercore local envelope must stay within one zone shard")
		}
		return nil
	}
	if e.SourceZone == e.DestinationZone {
		return errors.New("aethercore routed envelope requires cross-zone source")
	}
	if e.CommittedHeight == 0 || e.EligibleHeight == 0 {
		return errors.New("aethercore routed envelope requires committed and eligible heights")
	}
	if e.EligibleHeight > ctx.Height {
		return errors.New("aethercore routed envelope is not yet eligible")
	}
	if e.CommittedHeight >= e.EligibleHeight {
		return errors.New("aethercore routed envelope eligible height must follow committed height")
	}
	if _, found := state.RootSnapshotAtHeight(e.CommittedHeight); !found {
		return errors.New("aethercore routed envelope missing committed root")
	}
	if _, found := state.ZoneCommitmentAtHeight(e.CommittedHeight, e.SourceZone); !found {
		return errors.New("aethercore routed envelope missing source zone commitment")
	}
	return nil
}

func (i KernelCleanupItem) Validate() error {
	if err := validatePolicyID("aethercore cleanup queue id", i.QueueID); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore cleanup item id", i.ItemID); err != nil {
		return err
	}
	if i.HeightDue == 0 {
		return errors.New("aethercore cleanup item height must be positive")
	}
	return ValidateHash("aethercore cleanup delete root", i.DeleteRoot)
}

func (r KernelABCICommitRecord) Validate() error {
	if r.Height == 0 {
		return errors.New("aethercore ABCI commit height must be positive")
	}
	if err := ValidateHash("aethercore ABCI commit app hash", r.AppHash); err != nil {
		return err
	}
	if err := ValidateHash("aethercore ABCI commit header hash", r.HeaderHash); err != nil {
		return err
	}
	if err := ValidateHash("aethercore ABCI commit global root", r.GlobalRoot); err != nil {
		return err
	}
	if err := ValidateHash("aethercore ABCI commit message root", r.MessageRoot); err != nil {
		return err
	}
	if err := ValidateHash("aethercore ABCI commit receipts root", r.ReceiptsRoot); err != nil {
		return err
	}
	if err := ValidateHash("aethercore ABCI commit hash", r.CommitHash); err != nil {
		return err
	}
	if expected := ComputeKernelABCICommitHash(r); expected != r.CommitHash {
		return fmt.Errorf("aethercore ABCI commit hash mismatch: expected %s", expected)
	}
	return nil
}
...
func ComputeKernelEnvelopeHash(envelope KernelMessageEnvelope) string {
	...
	return hashParts(
		"aetheris-aek-kernel-envelope-v1",
...
func ComputeKernelEnvelopeRoot(envelopes []KernelMessageEnvelope) string {
	...
	parts := []string{"aetheris-aek-kernel-envelope-root-v1", fmt.Sprint(len(ordered))}
...
func ComputeKernelRoutedMessageRoot(envelopes []KernelMessageEnvelope) string {
	...
	parts := []string{"aetheris-aek-kernel-routed-message-root-v1", fmt.Sprint(len(routed))}
...
func ComputeKernelWorkloadsRoot(workloads []KernelZoneWorkload) string {
	...
	parts := []string{"aetheris-aek-kernel-workloads-root-v1", fmt.Sprint(len(ordered))}
...
func ComputeKernelZoneGasRoot(zoneGas []KernelZoneGas) string {
	...
	parts := []string{"aetheris-aek-kernel-zone-gas-root-v1", fmt.Sprint(len(ordered))}
...
func ComputeKernelCleanupRoot(result KernelCleanupResult) string {
	...
	parts := []string{"aetheris-aek-kernel-cleanup-root-v1", fmt.Sprint(result.Height), fmt.Sprint(len(items))}
	for _, item := range items {
...
func validateKernelEnvelopes(ctx KernelConsensusContext, state CoreState, envelopes []KernelMessageEnvelope, limits KernelGasLimits) error {
	...
	for _, envelope := range normalizeKernelEnvelopes(envelopes) {
		...
		if limits.MaxZoneGas-currentZoneGas < envelope.GasLimit {
			return errors.New("aethercore kernel proposal exceeds zone gas")
		}
...
func validateKernelGasLimits(limits KernelGasLimits) error {
	if limits.MaxBlockGas == 0 {
		return errors.New("aethercore kernel max block gas must be positive")
	}
	if limits.MaxZoneGas == 0 {
		return errors.New("aethercore kernel max zone gas must be positive")
	}
	if limits.MaxZoneGas > limits.MaxBlockGas {
		return errors.New("aethercore kernel max zone gas must not exceed block gas")
	}
	return nil
}
