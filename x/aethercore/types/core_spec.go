...
func NewZoneDescriptorSpecWithParams(descriptor ZoneDescriptor, params AetherCoreParams) (ZoneDescriptorSpec, error) {
...
func (p CoreExecutionPhaseSpec) ValidateHash() error {
	...
	if p.PhaseSpecHash != expected {
		return fmt.Errorf("aethercore execution phase spec hash mismatch: expected %s", expected)
	}
	return nil
}
...
func (p CoreExecutionPhaseSpec) ValidateFormat() error {
	if p.Phase != KernelPhasePrepareProposal && p.Phase != KernelPhaseProcessProposal && p.Phase != KernelPhaseFinalizeBlock && p.Phase != KernelPhaseCommit {
		return fmt.Errorf("unknown aethercore execution phase %q", p.Phase)
	}
	if len(p.DeterministicWork) == 0 {
		return errors.New("aethercore execution phase requires deterministic work")
	}
	if len(p.RejectionChecks) == 0 {
		return errors.New("aethercore execution phase requires rejection checks")
	}
	if len(p.CommittedOutput) == 0 {
		return errors.New("aethercore execution phase requires committed output")
	}
	if err := validateCapabilitiesForField("aethercore execution phase work", p.DeterministicWork); err != nil {
		return err
	}
	if err := validateCapabilitiesForField("aethercore execution phase rejection", p.RejectionChecks); err != nil {
		return err
	}
	if err := validateCapabilitiesForField("aethercore execution phase output", p.CommittedOutput); err != nil {
		return err
	}
	if p.PhaseSpecHash != "" {
		return ValidateHash("aethercore execution phase spec hash", p.PhaseSpecHash)
	}
	return nil
}
...
func (p CoreExecutionPipelineSpec) ValidateHash() error {
	...
	if p.PipelineHash != expected {
		return fmt.Errorf("aethercore execution pipeline hash mismatch: expected %s", expected)
	}
	return nil
}
...
func (p CoreExecutionPipelineSpec) ValidateFormat() error {
	if len(p.Phases) != 4 {
		return errors.New("aethercore execution pipeline requires four ABCI phases")
	}
	...
	for i, phase := range p.Phases {
		...
		if phase.Phase != expected[i] {
			return fmt.Errorf("aethercore execution pipeline phase order mismatch at index %d", i)
		}
	...
	if p.PipelineHash != "" {
		return ValidateHash("aethercore execution pipeline hash", p.PipelineHash)
	}
	return nil
}
...
func ComputeCoreExecutionPhaseSpecHash(phase CoreExecutionPhaseSpec) string {
	phase = canonicalCoreExecutionPhaseSpec(phase)
	parts := []string{"aetheris-aek-core-execution-phase-v1", string(phase.Phase)}
...
func ComputeCoreExecutionPipelineHash(pipeline CoreExecutionPipelineSpec) string {
	parts := []string{"aetheris-aek-core-execution-pipeline-v1", fmt.Sprint(len(pipeline.Phases))}
