...
func ValidateProofRootForObjective(objective UniversalProofObjective, root ProofRoot) error {
	...
	if !found {
		return fmt.Errorf("aethercore universal proof objective %q is not supported", objective)
	}
	if err := root.Validate(); err != nil {
		return err
	}
	if root.RootType != descriptor.RootType {
		return fmt.Errorf("aethercore proof objective %s requires root type %s", objective, descriptor.RootType)
	}
	if descriptor.ZoneID != "" && root.ZoneID != descriptor.ZoneID {
		return fmt.Errorf("aethercore proof objective %s requires zone %s", objective, descriptor.ZoneID)
	}
	return nil
}
...
func (b UniversalShardRootBranch) Validate(zoneID ZoneID) error {
	if b.ZoneID != zoneID {
		return fmt.Errorf("aethercore universal shard branch zone mismatch: expected %s", zoneID)
	}
	...
	return ValidateHash("aethercore universal shard root", b.ShardRoot)
}
...
func (b UniversalZoneRootBranch) Validate(height uint64) error {
	...
	if err := ValidateHash("aethercore universal zone root", b.ZoneRoot); err != nil {
		return err
	}
	if err := ValidateHash("aethercore universal shard roots root", b.ShardRootsRoot); err != nil {
		return err
	}
	...
	if b.ShardRootsRoot != expected {
		return fmt.Errorf("aethercore universal shard roots root mismatch: expected %s", expected)
	}
	return nil
}

func (b UniversalMessageRootBranch) ValidateFormat() error {
	if err := ValidateHash("aethercore universal zone outbox root", b.ZoneOutboxRoot); err != nil {
		return err
	}
	if err := ValidateHash("aethercore universal zone inbox root", b.ZoneInboxRoot); err != nil {
		return err
	}
	if err := ValidateHash("aethercore universal receipt root", b.ReceiptRoot); err != nil {
		return err
	}
	if b.MessageRoot != "" {
		return ValidateHash("aethercore universal message root", b.MessageRoot)
	}
	return nil
}
...
func (b UniversalMessageRootBranch) Validate(height uint64) error {
	if height == 0 {
		return errors.New("aethercore universal message root height must be positive")
	}
	...
	if b.MessageRoot != expected {
		return fmt.Errorf("aethercore universal message root mismatch: expected %s", expected)
	}
	return nil
}
...
func (h UniversalRootHierarchy) ValidateFormatOnly() error {
	if h.Height == 0 {
		return errors.New("aethercore universal root hierarchy height must be positive")
	}
	if err := ValidateHash("aethercore universal aether core root", h.AetherCoreRoot); err != nil {
		return err
	}
	if len(h.Zones) == 0 {
		return errors.New("aethercore universal root hierarchy requires zone roots")
	}
...
func (h UniversalRootHierarchy) Validate() error {
	...
	if h.GlobalZoneRoot != expectedZoneRoot {
		return fmt.Errorf("aethercore universal global zone root mismatch: expected %s", expectedZoneRoot)
	}
	expectedMessageRoot := ComputeUniversalMessageRoot(h.Height, h.Messages)
	if h.GlobalMessageRoot != expectedMessageRoot {
		return fmt.Errorf("aethercore universal global message root mismatch: expected %s", expectedMessageRoot)
	}
	expectedAppHash := ComputeUniversalAppHash(h)
	if h.AppHash != expectedAppHash {
		return fmt.Errorf("aethercore universal app hash mismatch: expected %s", expectedAppHash)
	}
	return nil
}
...
func (h UniversalRootHierarchy) ValidateRequiredZones(required []ZoneID) error {
	...
	for _, zoneID := range required {
		...
		if _, found := seen[zoneID]; !found {
			return fmt.Errorf("aethercore universal root hierarchy missing required zone %s", zoneID)
		}
	}
	return nil
}
...
func ComputeUniversalShardRootsRoot(height uint64, zoneID ZoneID, shards []UniversalShardRootBranch) (string, error) {
	if height == 0 {
		return "", errors.New("aethercore universal shard roots height must be positive")
	}
	if err := ValidateZoneID(zoneID); err != nil {
		return "", err
	}
	if len(shards) == 0 {
		return "", errors.New("aethercore universal shard roots require at least one shard")
	}
	ordered := canonicalUniversalShardRootBranches(shards)
	seen := make(map[ShardID]struct{}, len(ordered))
	return hashRoot("aetheris-next-universal-shard-roots-v1", func(w byteWriter) {
...
func ComputeUniversalGlobalZoneRoot(height uint64, zones []UniversalZoneRootBranch) (string, error) {
	if height == 0 {
		return "", errors.New("aethercore universal global zone root height must be positive")
	}
	if len(zones) == 0 {
		return "", errors.New("aethercore universal global zone root requires zone roots")
	}
	ordered := canonicalUniversalZoneRootBranches(zones)
	if err := validateUniversalZoneRootBranches(height, ordered); err != nil {
		return "", err
	}
	return hashRoot("aetheris-next-universal-global-zone-root-v1", func(w byteWriter) {
		writeUint64(w, height)
		writeUint64(w, uint64(len(ordered)))
...
func ComputeUniversalMessageRoot(height uint64, branch UniversalMessageRootBranch) string {
	return hashRoot("aetheris-next-universal-message-root-v1", func(w byteWriter) {
		writeUint64(w, height)
...
func ComputeUniversalAppHash(h UniversalRootHierarchy) string {
	return hashRoot("aetheris-next-universal-app-hash-v1", func(w byteWriter) {
		writeUint64(w, h.Height)
		writePart(w, h.AetherCoreRoot)
...
func validateUniversalShardRootBranches(zoneID ZoneID, shards []UniversalShardRootBranch) error {
	...
	for i, shard := range shards {
		...
		if _, found := seen[shard.ShardID]; found {
			return fmt.Errorf("duplicate aethercore universal shard root %s", shard.ShardID)
		}
		seen[shard.ShardID] = struct{}{}
		if i > 0 && previous >= shard.ShardID {
			return errors.New("aethercore universal shard roots must be sorted canonically")
		}
...
func validateUniversalZoneRootBranches(height uint64, zones []UniversalZoneRootBranch) error {
	...
	for i, zone := range zones {
		...
		if _, found := seen[zone.ZoneID]; found {
			return fmt.Errorf("duplicate aethercore universal zone root %s", zone.ZoneID)
		}
		seen[zone.ZoneID] = struct{}{}
		if i > 0 && previous >= zone.ZoneID {
			return errors.New("aethercore universal zone roots must be sorted canonically")
		}
