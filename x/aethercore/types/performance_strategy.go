...
func BuildBlockSTMZonePerformancePlan(schedule ProposalSchedule, accesses []BlockSTMStateAccess, batches []BlockSTMMessageBatch) (BlockSTMZonePerformancePlan, error) {
	...
	for _, access := range accesses {
		...
		if access.Mode == BlockSTMAccessWrite && access.StateZoneID != access.ActorZoneID && !access.ViaMessage {
			return BlockSTMZonePerformancePlan{}, errors.New("aethercore BlockSTM cross-zone writes must be converted into output messages")
		}
		if access.IsGlobalWriteLock() {
			return BlockSTMZonePerformancePlan{}, errors.New("aethercore BlockSTM workload cannot take global write lock")
		}
	}
...
func (p BlockSTMZonePerformancePlan) Validate() error {
	p = p.Normalize()
	if p.Height == 0 {
		return errors.New("aethercore BlockSTM performance height must be positive")
	}
	if len(p.Workloads) == 0 {
		return errors.New("aethercore BlockSTM performance plan requires workloads")
	}
	if p.GlobalWriteLocks != 0 {
		return errors.New("aethercore BlockSTM performance plan contains global write locks")
	}
	if p.ParallelWorkloads != uint32(len(p.Workloads)) {
		return errors.New("aethercore BlockSTM parallel workload count mismatch")
	}
	var previous string
	...
	for i, workload := range p.Workloads {
		...
		if _, found := seen[workload.WorkloadID]; found {
			return fmt.Errorf("duplicate aethercore BlockSTM workload %s", workload.WorkloadID)
		}
		seen[workload.WorkloadID] = struct{}{}
		if i > 0 && previous >= workload.WorkloadID {
			return errors.New("aethercore BlockSTM workloads must be sorted canonically")
		}
	...
	if err := ValidateHash("aethercore BlockSTM performance plan hash", p.PlanHash); err != nil {
		return err
	}
	if expected := ComputeBlockSTMZonePerformancePlanHash(p); p.PlanHash != expected {
		return fmt.Errorf("aethercore BlockSTM performance plan hash mismatch: expected %s", expected)
	}
	return nil
}
...
func (w BlockSTMZoneWorkload) Validate() error {
	w = w.Normalize()
	if err := ValidateHash("aethercore BlockSTM workload id", w.WorkloadID); err != nil {
		return err
	}
	...
	if len(w.Items) == 0 {
		return errors.New("aethercore BlockSTM workload requires proposal items")
	}
	for _, item := range w.Items {
		...
		if item.ZoneID != w.ZoneID || item.ShardID != w.ShardID {
			return errors.New("aethercore BlockSTM workload proposal item route mismatch")
		}
	}
	for _, access := range w.StateAccesses {
		...
		if access.ActorZoneID != w.ZoneID || access.ActorShardID != w.ShardID {
			return errors.New("aethercore BlockSTM workload access route mismatch")
		}
	}
	for _, batch := range w.MessageBatches {
		...
		if batch.SourceZoneID != w.ZoneID || batch.SourceShardID != w.ShardID {
			return errors.New("aethercore BlockSTM workload message batch route mismatch")
		}
	}
	if err := ValidateHash("aethercore BlockSTM workload conflict key root", w.ConflictKeyRoot); err != nil {
		return err
	}
	if w.ConflictKeyRoot != ComputeBlockSTMConflictKeyRoot(w.StateAccesses) {
		return errors.New("aethercore BlockSTM workload conflict key root mismatch")
	}
	if err := ValidateHash("aethercore BlockSTM workload message batch root", w.MessageBatchRoot); err != nil {
		return err
	}
	if w.MessageBatchRoot != ComputeBlockSTMMessageBatchRoot(w.MessageBatches) {
		return errors.New("aethercore BlockSTM workload message batch root mismatch")
	}
	if w.WorkloadID != ComputeBlockSTMWorkloadID(w) {
		return errors.New("aethercore BlockSTM workload id mismatch")
	}
	return nil
}
...
func (a BlockSTMStateAccess) Validate() error {
	...
	if err := validateToken("aethercore BlockSTM state key", a.StateKey, MaxScopeLength); err != nil {
		return err
	}
	if !IsBlockSTMAccessMode(a.Mode) {
		return fmt.Errorf("unknown aethercore BlockSTM access mode %q", a.Mode)
	}
	return nil
}
...
func (a BlockSTMStateAccess) IsGlobalWriteLock() bool {
	...
	return a.StateZoneID == ZoneIDAetherCore && (a.StateKey == "core/global-lock" || a.StateKey == "core/*" || a.StateKey == "*")
}
...
func (b BlockSTMMessageBatch) Validate() error {
	...
	if b.MessageCount == 0 {
		return errors.New("aethercore BlockSTM message batch count must be positive")
	}
	if err := ValidateHash("aethercore BlockSTM message batch hash", b.BatchHash); err != nil {
		return err
	}
	if b.BatchHash != ComputeBlockSTMMessageBatchHash(b) {
		return errors.New("aethercore BlockSTM message batch hash mismatch")
	}
	return nil
}
...
func (c BlockSTMConflictSet) Validate() error {
	if c.ConflictKey == "" {
		return errors.New("aethercore BlockSTM conflict key is required")
	}
	if len(c.WorkloadIDs) < 2 {
		return errors.New("aethercore BlockSTM conflict set requires at least two workloads")
	}
	seen := make(map[string]struct{}, len(c.WorkloadIDs))
	var previous string
	for i, id := range c.WorkloadIDs {
		if err := ValidateHash("aethercore BlockSTM conflict workload id", id); err != nil {
			return err
		}
		if _, found := seen[id]; found {
			return fmt.Errorf("duplicate aethercore BlockSTM conflict workload %s", id)
		}
		seen[id] = struct{}{}
		if i > 0 && previous >= id {
			return errors.New("aethercore BlockSTM conflict workloads must be sorted canonically")
		}
		previous = id
	}
	return nil
...
func ComputeBlockSTMWorkloadID(workload BlockSTMZoneWorkload) string {
	workload = workload.Normalize()
	parts := []string{"aetheris-blockstm-zone-workload-v1", string(workload.ZoneID), string(workload.ShardID), fmt.Sprint(len(workload.Items))}
...
func ComputeBlockSTMConflictKeyRoot(accesses []BlockSTMStateAccess) string {
	...
	parts := []string{"aetheris-blockstm-conflict-key-root-v1", fmt.Sprint(len(ordered))}
...
func ComputeBlockSTMMessageBatchHash(batch BlockSTMMessageBatch) string {
	...
	return hashParts(
		"aetheris-blockstm-message-batch-v1",
...
func ComputeBlockSTMMessageBatchRoot(batches []BlockSTMMessageBatch) string {
	...
	parts := []string{"aetheris-blockstm-message-batch-root-v1", fmt.Sprint(len(ordered))}
...
func ComputeBlockSTMZonePerformancePlanHash(plan BlockSTMZonePerformancePlan) string {
	plan = plan.Normalize()
	parts := []string{
		"aetheris-blockstm-zone-performance-plan-v1",
