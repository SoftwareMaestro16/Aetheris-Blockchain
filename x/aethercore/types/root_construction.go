...
func (c RootContribution) ValidateFormat() error {
	...
	if err := validatePolicyID("aethercore root contribution type", string(c.RootType)); err != nil {
		return err
	}
	if err := validateToken("aethercore root contribution id", c.ID, MaxScopeLength); err != nil {
		return err
	}
	if err := ValidateHash("aethercore root contribution hash", c.RootHash); err != nil {
		return err
	}
	if c.ContributionHash != "" {
		if err := ValidateHash("aethercore root contribution commitment", c.ContributionHash); err != nil {
			return err
		}
	}
	return nil
...
func (c RootContribution) Validate() error {
	...
	if c.ContributionHash != ComputeRootContributionHash(c) {
		return errors.New("aethercore root contribution commitment mismatch")
	}
	return nil
}
...
func (a ZoneRootAggregation) ValidateFormat() error {
	...
	if a.Height == 0 {
		return errors.New("aethercore zone root aggregation height must be positive")
	}
	if err := ValidateZoneID(a.ZoneID); err != nil {
		return err
	}
	return validateRootContributionSet("aethercore zone module roots", a.ModuleRoots)
}
...
func (a ZoneRootAggregation) Validate() error {
	...
	if err := ValidateHash("aethercore zone aggregate root", a.ZoneRoot); err != nil {
		return err
	}
	if a.ZoneRoot != ComputeZoneRootAggregationHash(a) {
		return errors.New("aethercore zone aggregate root mismatch")
	}
	return nil
}
...
func (a AEKRootAggregation) ValidateFormat() error {
	...
	if a.Height == 0 {
		return errors.New("aethercore AEK root aggregation height must be positive")
	}
	if err := validateRootContributionSet("aethercore AEK zone roots", a.ZoneRoots); err != nil {
		return err
	}
	return validateRootContributionSet("aethercore AEK global roots", a.GlobalRoots)
}
...
func (a AEKRootAggregation) Validate() error {
	...
	if err := ValidateHash("aethercore AEK aggregate root", a.AggregateRoot); err != nil {
		return err
	}
	if a.AggregateRoot != ComputeAEKRootAggregationHash(a) {
		return errors.New("aethercore AEK aggregate root mismatch")
	}
	return nil
}
...
func (p StateCommitmentProof) ValidateFormat() error {
	...
	if !IsCommitmentProofType(p.ProofType) {
		return fmt.Errorf("unknown aethercore proof type %q", p.ProofType)
	}
	if p.Height == 0 {
		return errors.New("aethercore proof height must be positive")
	}
	if err := validatePolicyID("aethercore proof root type", string(p.RootType)); err != nil {
		return err
	}
	if err := validateToken("aethercore proof subject id", p.SubjectID, MaxScopeLength); err != nil {
		return err
	}
	if err := ValidateHash("aethercore proof key hash", p.KeyHash); err != nil {
		return err
	}
	if err := ValidateHash("aethercore proof value hash", p.ValueHash); err != nil {
		return err
	}
	if err := ValidateHash("aethercore proof root hash", p.RootHash); err != nil {
		return err
	}
	for _, item := range p.Path {
		if err := ValidateHash("aethercore proof path item", item); err != nil {
			return err
		}
	}
	if p.ProofType == NonExistenceProofType && p.Exists {
		return errors.New("aethercore non-existence proof must not claim existence")
	}
	if p.ProofType != NonExistenceProofType && !p.Exists {
		return errors.New("aethercore existence proof type must claim existence")
	}
	if p.ProofHash != "" {
		if err := ValidateHash("aethercore proof hash", p.ProofHash); err != nil {
			return err
		}
	}
	return nil
...
func (p StateCommitmentProof) Validate() error {
	...
	if p.ProofHash != ComputeStateCommitmentProofHash(p) {
		return errors.New("aethercore proof hash mismatch")
	}
	return nil
}
...
func ComputeRootContributionHash(c RootContribution) string {
	c = normalizeRootContribution(c)
	return hashRoot("aetheris-aek-root-contribution-v1", func(w byteWriter) {
...
func ComputeZoneRootAggregationHash(a ZoneRootAggregation) string {
	a = normalizeZoneRootAggregation(a)
	return hashRoot("aetheris-aek-zone-root-aggregation-v1", func(w byteWriter) {
...
func ComputeAEKRootAggregationHash(a AEKRootAggregation) string {
	a = normalizeAEKRootAggregation(a)
	return hashRoot("aetheris-aek-root-aggregation-v1", func(w byteWriter) {
		writeUint64(w, a.Height)
...
func ComputeStateCommitmentProofHash(p StateCommitmentProof) string {
	p = normalizeStateCommitmentProof(p)
	return hashRoot("aetheris-aek-state-commitment-proof-v1", func(w byteWriter) {
...
func DeterministicEmptyRootCommitment(rootType RootType, id string) string {
	return hashRoot("aetheris-aek-empty-root-commitment-v1", func(w byteWriter) {
