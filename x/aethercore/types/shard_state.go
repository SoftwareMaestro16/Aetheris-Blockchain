...
func NewShardRebalanceDecision(layout ShardLayout, metrics []ShardMetrics, thresholds ShardRebalanceThresholds, decisionHeight uint64) (ShardRebalanceDecision, error) {
	...
	if decisionHeight == 0 {
		return ShardRebalanceDecision{}, errors.New("aethercore shard rebalance decision height must be positive")
	}
	...
	if len(ordered) == 0 {
		return ShardRebalanceDecision{}, errors.New("aethercore shard rebalance requires committed metrics")
	}
	for _, metric := range ordered {
		...
		if metric.ZoneID != layout.ZoneID {
			return ShardRebalanceDecision{}, errors.New("aethercore shard rebalance metrics zone mismatch")
		}
		if !layout.HasActiveShard(metric.ShardID) {
			return ShardRebalanceDecision{}, errors.New("aethercore shard rebalance metrics shard is not active")
		}
		if metric.Height > decisionHeight {
			return ShardRebalanceDecision{}, errors.New("aethercore shard rebalance cannot use future metrics")
		}
	}
...
func (t ShardRebalanceThresholds) Validate() error {
	if t.GasLimitPerShard == 0 {
		return errors.New("aethercore shard rebalance gas limit must be positive")
	}
	if t.SplitGasUtilization == 0 {
		return errors.New("aethercore shard split gas utilization threshold must be positive")
	}
	if t.MergeGasUtilization > t.SplitGasUtilization {
		return errors.New("aethercore shard merge gas threshold must not exceed split threshold")
	}
	if t.DecisionWindow == 0 {
		return errors.New("aethercore shard rebalance decision window must be positive")
	}
	if t.FutureLayoutEpochDelta == 0 {
		return errors.New("aethercore shard rebalance future layout epoch delta must be positive")
	}
	if t.FutureActivationHeightGap == 0 {
		return errors.New("aethercore shard rebalance activation height gap must be positive")
	}
	return nil
}
...
func (d ShardRebalanceDecision) ValidateHash() error {
	...
	if d.DecisionHash != ComputeShardRebalanceDecisionHash(d) {
		return errors.New("aethercore shard rebalance decision hash mismatch")
	}
	return nil
}
...
func (d ShardRebalanceDecision) ValidateFormat() error {
	...
	if d.SourceLayoutEpoch == 0 || d.TargetLayoutEpoch == 0 {
		return errors.New("aethercore shard rebalance layout epochs must be positive")
	}
	if d.TargetLayoutEpoch <= d.SourceLayoutEpoch {
		return errors.New("aethercore shard rebalance target epoch must be future")
	}
	if d.DecisionHeight == 0 || d.ActivationHeight == 0 {
		return errors.New("aethercore shard rebalance heights must be positive")
	}
	if d.ActivationHeight <= d.DecisionHeight {
		return errors.New("aethercore shard rebalance activation height must be future")
	}
	if !IsShardLayoutChangeKind(d.ChangeKind) {
		return fmt.Errorf("unknown aethercore shard layout change kind %q", d.ChangeKind)
	}
	if d.ChangeKind != ShardLayoutChangeNone {
		if err := validateToken("aethercore shard rebalance reason", d.Reason, MaxScopeLength); err != nil {
			return err
		}
	}
	if err := ValidateHash("aethercore shard rebalance metrics hash", d.SourceMetricsHash); err != nil {
		return err
	}
	if err := ValidateHash("aethercore shard rebalance source layout hash", d.SourceLayoutHash); err != nil {
		return err
	}
	if err := validateShardIDList("aethercore shard rebalance source shard", d.SourceShardIDs); err != nil {
		return err
	}
	if err := validateShardIDList("aethercore shard rebalance target shard", d.TargetShardIDs); err != nil {
		return err
	}
	...
	if d.DecisionHash != "" {
		return ValidateHash("aethercore shard rebalance decision hash", d.DecisionHash)
	}
	return nil
}
...
func NewShardMigrationTask(task ShardMigrationTask) (ShardMigrationTask, error) {
	if task.TaskHash != "" || task.TaskID != "" {
		return ShardMigrationTask{}, errors.New("aethercore shard migration task id and hash must be empty before construction")
	}
...
func (t ShardMigrationTask) ValidateHash() error {
	...
	if t.TaskHash != ComputeShardMigrationTaskHash(t) {
		return errors.New("aethercore shard migration task hash mismatch")
	}
	if t.TaskID != t.TaskHash[:16] {
		return errors.New("aethercore shard migration task id mismatch")
	}
	return nil
}
...
func (t ShardMigrationTask) ValidateFormat() error {
	...
	if t.SourceShardID == t.DestinationShardID {
		return errors.New("aethercore shard migration source and destination must differ")
	}
	if t.SourceLayoutEpoch == 0 || t.TargetLayoutEpoch == 0 || t.DeliveryEpoch == 0 {
		return errors.New("aethercore shard migration epochs must be positive")
	}
	if t.TargetLayoutEpoch <= t.SourceLayoutEpoch {
		return errors.New("aethercore shard migration target epoch must be future")
	}
	if t.DeliveryEpoch < t.TargetLayoutEpoch {
		return errors.New("aethercore shard migration delivery epoch must not precede target layout epoch")
	}
	if t.KeyPrefix != "" {
		if err := validateToken("aethercore shard migration key prefix", t.KeyPrefix, MaxScopeLength); err != nil {
			return err
		}
	}
	if t.ObjectKey != "" {
		if err := validateToken("aethercore shard migration object key", t.ObjectKey, MaxScopeLength); err != nil {
			return err
		}
	}
	if t.HashRangeEnd < t.HashRangeStart {
		return errors.New("aethercore shard migration hash range end must not precede start")
	}
	if t.TaskID != "" {
		if err := validateToken("aethercore shard migration task id", t.TaskID, MaxIDLength); err != nil {
			return err
		}
	}
	if t.TaskHash != "" {
		return ValidateHash("aethercore shard migration task hash", t.TaskHash)
	}
	return nil
}
...
func NewObjectLock(lock ObjectLock) (ObjectLock, error) {
	if lock.LockHash != "" || lock.LockID != "" {
		return ObjectLock{}, errors.New("aethercore object lock id and hash must be empty before construction")
	}
...
func (l ObjectLock) ValidateHash() error {
	...
	if l.LockHash != ComputeObjectLockHash(l) {
		return errors.New("aethercore object lock hash mismatch")
	}
	if l.LockID != l.LockHash[:16] {
		return errors.New("aethercore object lock id mismatch")
	}
	return nil
}
...
func (l ObjectLock) ValidateFormat() error {
	...
	if err := validateToken("aethercore object lock object id", l.ObjectID, MaxScopeLength); err != nil {
		return err
	}
	if err := validateToken("aethercore object lock reason", l.Reason, MaxScopeLength); err != nil {
		return err
	}
	if err := validateToken("aethercore object lock owner task id", l.OwnerTaskID, MaxIDLength); err != nil {
		return err
	}
	if l.CreatedHeight == 0 || l.ExpiryHeight == 0 {
		return errors.New("aethercore object lock heights must be positive")
	}
	if l.ExpiryHeight <= l.CreatedHeight {
		return errors.New("aethercore object lock expiry height must be future")
	}
	if l.LockID != "" {
		if err := validateToken("aethercore object lock id", l.LockID, MaxIDLength); err != nil {
			return err
		}
	}
	if l.LockHash != "" {
		return ValidateHash("aethercore object lock hash", l.LockHash)
	}
	return nil
}
...
func NewShardRoot(root ShardRoot) (ShardRoot, error) {
	if root.RootHash != "" {
		return ShardRoot{}, errors.New("aethercore shard root hash must be empty before construction")
	}
...
func (r ShardRoot) ValidateHash() error {
	...
	if r.RootHash != ComputeShardRootHash(r) {
		return errors.New("aethercore shard root hash mismatch")
	}
	return nil
}
...
func (r ShardRoot) ValidateFormat() error {
	...
	if r.Height == 0 {
		return errors.New("aethercore shard root height must be positive")
	}
	for field, value := range map[string]string{
		...
		if err := ValidateHash("aethercore shard "+field+" root", value); err != nil {
			return err
		}
	}
	if r.RootHash != "" {
		return ValidateHash("aethercore shard root hash", r.RootHash)
	}
	return nil
}
...
func ComputeShardMetricsWindowHash(metrics []ShardMetrics) string {
	ordered := cloneShardMetrics(metrics)
	sortShardMetrics(ordered)
	parts := []string{"aetheris-aek-shard-metrics-window-v1", fmt.Sprint(len(ordered))}
	for _, metric := range ordered {
...
func ComputeShardRebalanceDecisionHash(decision ShardRebalanceDecision) string {
	...
	parts := []string{
		"aetheris-aek-shard-rebalance-decision-v1",
...
func ComputeShardMigrationTaskHash(task ShardMigrationTask) string {
	return hashParts(
		"aetheris-aek-shard-migration-task-v1",
		string(task.ZoneID),
...
func ComputeShardRootsRoot(roots []ShardRoot) (string, error) {
	...
	if len(ordered) == 0 {
		return "", errors.New("aethercore shard roots root requires shard roots")
	}
	parts := []string{"aetheris-aek-shard-roots-root-v1", fmt.Sprint(len(ordered))}
...
func validateShardMigrationTasks(tasks []ShardMigrationTask, decision ShardRebalanceDecision) error {
	...
	for _, task := range ordered {
		...
		if task.ZoneID != decision.ZoneID {
			return errors.New("aethercore shard migration task zone mismatch")
		}
		if task.SourceLayoutEpoch != decision.SourceLayoutEpoch || task.TargetLayoutEpoch != decision.TargetLayoutEpoch {
			return errors.New("aethercore shard migration task epoch mismatch")
		}
	}
	return nil
}
