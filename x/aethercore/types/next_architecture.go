...
func (c AetherisNextCommitment) ValidateFormat() error {
	...
	for _, field := range []struct {
		...
		{"aethercore next zone descriptor root", c.ZoneDescriptorRoot},
		{"aethercore next zone commitments root", c.ZoneCommitmentsRoot},
		{"aethercore next shard layout root", c.ShardLayoutRoot},
		{"aethercore next routing table root", c.RoutingTableRoot},
		{"aethercore next universal proof root", c.UniversalProofRoot},
		{"aethercore next global state root", c.GlobalStateRoot},
		{"aethercore next params hash", c.ParamsHash},
	} {
	...
	if c.ArchitectureHash != "" {
		return ValidateHash("aethercore next architecture hash", c.ArchitectureHash)
	}
	return nil
}
...
func ComputeActiveShardLayoutRoot(height uint64, layouts []ShardLayout) (string, error) {
	if height == 0 {
		return "", errors.New("aethercore active shard layout root height must be positive")
	}
	ordered := cloneShardLayouts(layouts)
	sortShardLayouts(ordered)
	parts := []string{"aetheris-next-active-shard-layout-root-v1", fmt.Sprint(height), fmt.Sprint(len(ordered))}
	...
	for i, layout := range ordered {
		...
		if layout.ActivationHeight > height {
			return "", errors.New("aethercore active shard layout activates after root height")
		}
		if i > 0 && compareShardLayouts(previous, layout) >= 0 {
			return "", errors.New("aethercore active shard layouts must be sorted canonically")
		}
...
func ComputeUniversalProofRegistryRoot(height uint64, roots []ProofRoot) (string, error) {
	if height == 0 {
		return "", errors.New("aethercore universal proof registry height must be positive")
	}
	ordered := append([]ProofRoot(nil), roots...)
	sortProofRoots(ordered)
	parts := []string{"aetheris-next-universal-proof-registry-v1", fmt.Sprint(height), fmt.Sprint(len(ordered))}
	var previous ProofRoot
	for i, root := range ordered {
		if root.Height != height {
			return "", errors.New("aethercore universal proof registry contains different height")
		}
		if err := root.Validate(); err != nil {
			return "", err
		}
		if i > 0 && compareProofRoots(previous, root) >= 0 {
			return "", errors.New("aethercore universal proof roots must be sorted canonically")
		}
		parts = append(parts,
...
func latestExecutionZoneLayouts(height uint64, state CoreState, zones []ZoneDescriptor) ([]ShardLayout, error) {
	...
	for _, zone := range zones {
		...
		if !zone.Enabled || zone.ZoneID == ZoneIDAetherCore {
			continue
		}
		layout, found := state.LatestShardLayout(zone.ZoneID, height)
		if !found {
			return nil, fmt.Errorf("aethercore next architecture missing shard layout for zone %s", zone.ZoneID)
		}
...
func validateRoutingTableCoversLayouts(table RoutingTableCommitment, layouts []ShardLayout) error {
	...
	for _, layout := range layouts {
		...
		if !found {
			return fmt.Errorf("aethercore routing table missing zone %s", layout.ZoneID)
		}
		if entry.LayoutEpoch != layout.LayoutEpoch {
			return fmt.Errorf("aethercore routing table layout epoch mismatch for zone %s", layout.ZoneID)
		}
		if entry.LayoutHash != layout.LayoutHash {
			return fmt.Errorf("aethercore routing table layout hash mismatch for zone %s", layout.ZoneID)
		}
		if entry.ActiveShards != uint32(len(layout.ActiveShards)) {
			return fmt.Errorf("aethercore routing table active shard mismatch for zone %s", layout.ZoneID)
		}
	}
	return nil
}
...
func ensureEnabledZoneCommitmentsAtHeight(height uint64, zones []ZoneDescriptor, commitments []ZoneCommitment) error {
	...
	for _, zone := range zones {
		...
		if _, found := seen[zone.ZoneID]; !found {
			return fmt.Errorf("aethercore next architecture missing zone commitment for %s", zone.ZoneID)
		}
	}
	return nil
}
...
func ensureEnabledZones(zones []ZoneDescriptor) error {
	...
	return errors.New("aethercore next architecture requires at least one enabled zone")
}
...
func ensureNativeAETResolver(state CoreState) error {
	...
	if !found || !identity.Enabled {
		return errors.New("aethercore next architecture requires enabled identity zone for .aet resolver")
	}
	...
	return errors.New("aethercore next architecture requires native .aet identity resolver service")
}
...
func nextArchitectureProofRoots(
	...
	return []ProofRoot{
		{Height: height, RootType: StateProofRootType, RootHash: globalStateRoot, Source: "aethercore.next.state"},
		{Height: height, RootType: ZoneCommitmentsRoot, RootHash: zoneCommitmentsRoot, Source: "aethercore.next.zones"},
		{Height: height, RootType: ShardLayoutRootType, RootHash: shardLayoutRoot, Source: "aethercore.next.shards"},
		{Height: height, RootType: RoutingTableRootType, RootHash: routingTableRoot, Source: "aethercore.next.routing"},
		{Height: height, RootType: AccountProofRootType, RootHash: contributions.StorageRoot, Source: "aethercore.next.accounts"},
		{Height: height, RootType: MessageProofRootType, RootHash: contributions.MessageRoot, Source: "aethercore.next.messages"},
		{Height: height, RootType: ReceiptProofRootType, RootHash: contributions.ReceiptsRoot, Source: "aethercore.next.receipts"},
		{Height: height, RootType: IdentityProofRootType, RootHash: contributions.IdentityRoot, Source: "aethercore.next.identity"},
		{Height: height, RootType: ResolverProofRootType, RootHash: resolverRoot, Source: "aethercore.next.resolver"},
		{Height: height, RootType: PaymentsProofRootType, RootHash: contributions.PaymentsRoot, Source: "aethercore.next.payments"},
		{Height: height, RootType: VMProofRootType, RootHash: contributions.VMRoot, Source: "aethercore.next.vm"},
	}
}
