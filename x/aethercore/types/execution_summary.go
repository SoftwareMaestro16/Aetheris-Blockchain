...
func CollectZoneExecutionSummary(height uint64, zoneID ZoneID, envelopes []KernelMessageEnvelope, receipts []ExecutionReceipt, gasUsed uint64, eventsRoot string) (ZoneExecutionSummary, error) {
	if height == 0 {
		return ZoneExecutionSummary{}, errors.New("aethercore execution summary height must be positive")
	}
	...
	if err := ValidateHash("aethercore execution summary events root", eventsRoot); err != nil {
		return ZoneExecutionSummary{}, err
	}
...
func (s ZoneExecutionSummary) Validate() error {
	if s.Height == 0 {
		return errors.New("aethercore execution summary height must be positive")
	}
	if err := ValidateZoneID(s.ZoneID); err != nil {
		return err
	}
	if err := ValidateHash("aethercore execution summary message root", s.MessageRoot); err != nil {
		return err
	}
	if err := ValidateHash("aethercore execution summary receipts root", s.ReceiptsRoot); err != nil {
		return err
	}
	if err := ValidateHash("aethercore execution summary events root", s.EventsRoot); err != nil {
		return err
	}
	if s.FailedReceiptCount > s.ReceiptCount {
		return errors.New("aethercore execution summary failed receipts exceed receipt count")
	}
	if err := ValidateHash("aethercore execution summary hash", s.SummaryHash); err != nil {
		return err
	}
	if expected := ComputeZoneExecutionSummaryHash(s); expected != s.SummaryHash {
		return fmt.Errorf("aethercore execution summary hash mismatch: expected %s", expected)
	}
	return nil
}
...
func ComputeZoneExecutionSummaryHash(summary ZoneExecutionSummary) string {
	return hashParts(
		"aetheris-aek-zone-execution-summary-v1",
