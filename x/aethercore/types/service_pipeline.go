...
func FinalizeServiceProposal(ctx ServiceConsensusContext, state CoreState, plan ServiceProposalPlan, outcomes []ServiceExecutionOutcome) (ServiceFinalization, error) {
	...
	for _, outcome := range outcomes {
		...
		if _, found := outcomeByCall[outcome.CallID]; found {
			return ServiceFinalization{}, fmt.Errorf("duplicate aethercore service outcome %s", outcome.CallID)
		}
	...
	for _, call := range plan.OrderedCalls() {
		...
		if !found {
			return ServiceFinalization{}, fmt.Errorf("missing aethercore service outcome %s", call.CallID)
		}
...
func EndBlockServiceMaintenance(state CoreState, height uint64, receipts []ServiceCallReceipt, limit uint64) (ServiceEndBlockMaintenance, error) {
	if height == 0 {
		return ServiceEndBlockMaintenance{}, errors.New("aethercore service endblock height must be positive")
	}
	if limit == 0 {
		return ServiceEndBlockMaintenance{}, errors.New("aethercore service endblock cleanup limit must be positive")
	}
	if err := state.Validate(); err != nil {
...
func (ctx ServiceConsensusContext) Validate() error {
	if err := validatePolicyID("aethercore service chain id", ctx.ChainID); err != nil {
		return err
	}
	if ctx.Height == 0 {
		return errors.New("aethercore service consensus height must be positive")
	}
	return nil
}
...
func ValidateServiceCallForState(ctx ServiceConsensusContext, state CoreState, call ServiceCallEnvelope) error {
	...
	if !found {
		return fmt.Errorf("aethercore service call references unknown service %s", call.ServiceID)
	}
	if !descriptor.Enabled || descriptor.Status != ServiceStatusActive {
		return fmt.Errorf("aethercore service %s is not active", call.ServiceID)
	}
	if descriptor.ExpiryHeight != 0 && call.DeadlineHeight > descriptor.ExpiryHeight {
		return errors.New("aethercore service call deadline must not exceed service expiry")
	}
	if call.InterfaceHash != descriptor.Interface.InterfaceHash {
		return errors.New("aethercore service call interface hash mismatch")
	}
	method, found := descriptor.Interface.MethodByID(call.MethodID)
	if !found {
		return fmt.Errorf("aethercore service method %s is not registered", call.MethodID)
	}
	if method.IdempotencyRequired && call.IdempotencyKey == "" {
		return errors.New("aethercore service call requires idempotency key")
	}
	if call.ProofRequirement != method.VerificationModel {
		return errors.New("aethercore service call proof requirement mismatch")
	}
	if call.PaymentDenom != descriptor.Payment.Denom {
		return errors.New("aethercore service call payment denom mismatch")
	}
	if !serviceCallKindAllowed(descriptor.ServiceType, call.Kind) {
		return fmt.Errorf("aethercore service call kind %s is incompatible with %s service", call.Kind, descriptor.ServiceType)
	}
	return nil
}

func (call ServiceCallEnvelope) ValidateBasic(ctx ServiceConsensusContext) error {
	if err := validatePolicyID("aethercore service call service id", call.ServiceID); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore service call caller", call.Caller); err != nil {
		return err
	}
	if call.Nonce == 0 {
		return errors.New("aethercore service call nonce must be positive")
	}
	if err := validatePolicyID("aethercore service call method id", call.MethodID); err != nil {
		return err
	}
	if err := ValidateHash("aethercore service call interface hash", call.InterfaceHash); err != nil {
		return err
	}
	if err := ValidateHash("aethercore service call payload hash", call.PayloadHash); err != nil {
		return err
	}
	if call.CreatedHeight == 0 || call.CreatedHeight > ctx.Height {
		return errors.New("aethercore service call created height is invalid")
	}
	if call.DeadlineHeight == 0 || call.DeadlineHeight < ctx.Height {
		return errors.New("aethercore service call is expired")
	}
	if !IsServiceVerificationModel(call.ProofRequirement) {
		return fmt.Errorf("unknown aethercore service call proof requirement %q", call.ProofRequirement)
	}
	if !IsServiceCallKind(call.Kind) {
		return fmt.Errorf("unknown aethercore service call kind %q", call.Kind)
	}
	if err := validatePolicyID("aethercore service call payment denom", call.PaymentDenom); err != nil {
		return err
	}
	if err := validateAmountString("aethercore service call max fee", call.MaxFeeAmount); err != nil {
		return err
	}
	if call.RetryOf != "" {
		if err := ValidateHash("aethercore service retry call id", call.RetryOf); err != nil {
			return err
		}
	}
	if err := validateServiceStateKeys("aethercore service read set", call.StateReadSet); err != nil {
		return err
	}
	if err := validateServiceStateKeys("aethercore service write set", call.StateWriteSet); err != nil {
		return err
	}
	expected := ComputeServiceCallID(ctx, call)
	if call.CallID != expected {
		return fmt.Errorf("aethercore service call id mismatch: expected %s", expected)
	}
	return nil
}
...
func ValidateServiceProposalPlan(ctx ServiceConsensusContext, state CoreState, plan ServiceProposalPlan) error {
	...
	if plan.Height != ctx.Height || plan.ChainID != ctx.ChainID {
		return errors.New("aethercore service proposal context mismatch")
	}
	if err := ValidateHash("aethercore service proposal registry root", plan.RegistryRoot); err != nil {
		return err
	}
	if err := ValidateHash("aethercore service proposal interface root", plan.InterfaceRoot); err != nil {
		return err
	}
	if expected := ComputeServiceProposalPlanHash(plan); plan.PlanHash != expected {
		return fmt.Errorf("aethercore service proposal hash mismatch: expected %s", expected)
	}
	registryRoot, err := ComputeServiceRoot(state.ServiceDescriptors)
	if err != nil {
		return err
	...
	if registryRoot != plan.RegistryRoot {
		return errors.New("aethercore service proposal registry root mismatch")
	}
	interfaceRoot, err := ComputeServiceInterfaceRoot(state.ServiceDescriptors)
	if err != nil {
		return err
	...
	if interfaceRoot != plan.InterfaceRoot {
		return errors.New("aethercore service proposal interface root mismatch")
	}
	...
	for i, group := range plan.Groups {
		...
		if i > 0 && plan.Groups[i-1].GroupID >= group.GroupID {
			return errors.New("aethercore service proposal groups must be sorted canonically")
		}
		for _, call := range group.Calls {
			if _, found := seenCalls[call.CallID]; found {
				return fmt.Errorf("duplicate aethercore service call %s", call.CallID)
			}
			...
			if lastNonce := senderLastNonce[senderKey]; lastNonce >= call.Nonce {
				return errors.New("aethercore service calls from same sender must be nonce ordered")
			}
			...
			for _, key := range call.StateWriteSet {
				if owner, found := writeOwners[key]; found && owner != group.GroupID {
					return errors.New("aethercore service proposal groups have BlockSTM write conflict")
				}
...
func (g ServiceExecutionGroup) Validate(ctx ServiceConsensusContext, state CoreState) error {
	if err := ValidateHash("aethercore service execution group id", g.GroupID); err != nil {
		return err
	}
	if err := ValidateHash("aethercore service execution group scope", g.StateKeyScope); err != nil {
		return err
	}
	if g.GroupID != ComputeServiceExecutionGroupID(g.StateKeyScope) {
		return errors.New("aethercore service execution group id mismatch")
	}
	if len(g.Calls) == 0 {
		return errors.New("aethercore service execution group requires calls")
	}
	for i, call := range g.Calls {
		...
		if ComputeServiceCallStateScope(call) != g.StateKeyScope {
			return errors.New("aethercore service execution group scope mismatch")
		}
		if i > 0 && compareServiceCalls(g.Calls[i-1], call) >= 0 {
			return errors.New("aethercore service execution group calls must be sorted canonically")
		}
	}
	return nil
}
...
func ValidateServiceExecutionOutcome(ctx ServiceConsensusContext, call ServiceCallEnvelope, outcome ServiceExecutionOutcome) error {
	if outcome.CallID != call.CallID {
		return errors.New("aethercore service outcome call id mismatch")
	}
	if !IsServiceCallStatus(outcome.Status) {
		return fmt.Errorf("unknown aethercore service call status %q", outcome.Status)
	}
	if !IsServicePaymentStatus(outcome.PaymentStatus) {
		return fmt.Errorf("unknown aethercore service payment status %q", outcome.PaymentStatus)
	}
	if outcome.ResponseHash != "" {
		if err := ValidateHash("aethercore service response hash", outcome.ResponseHash); err != nil {
			return err
		}
	}
	if outcome.ProofHash != "" {
		if err := ValidateHash("aethercore service proof hash", outcome.ProofHash); err != nil {
			return err
		}
	}
	if outcome.ExecutedHeight == 0 || outcome.ExecutedHeight > ctx.Height {
		return errors.New("aethercore service outcome executed height is invalid")
	}
	if outcome.AnchoredHeight == 0 || outcome.AnchoredHeight > ctx.Height {
		return errors.New("aethercore service outcome anchored height is invalid")
	}
	if outcome.ProviderID != "" {
		if err := validatePolicyID("aethercore service provider id", outcome.ProviderID); err != nil {
			return err
		}
	}
	if outcome.ErrorCode != "" {
		if err := validatePolicyID("aethercore service error code", outcome.ErrorCode); err != nil {
			return err
		}
	}
	if call.Kind == ServiceCallKindOffChainReceipt && outcome.ProofHash == "" && call.ProofRequirement != ServiceVerificationSignedResult {
		return errors.New("aethercore off-chain service receipt requires proof hash unless signed-result verified")
	}
	if call.Kind == ServiceCallKindMixedDispute && outcome.Status != ServiceCallStatusChallenged && outcome.Status != ServiceCallStatusSettled {
		return errors.New("aethercore mixed dispute outcome must challenge or settle")
	}
	if call.Kind == ServiceCallKindMixedSettlement && outcome.Status != ServiceCallStatusSettled {
		return errors.New("aethercore mixed settlement outcome must settle")
	}
	return nil
}
...
func (r ServiceCallReceipt) Validate() error {
	if err := ValidateHash("aethercore service receipt call id", r.CallID); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore service receipt service id", r.ServiceID); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore service receipt method id", r.MethodID); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore service receipt caller", r.Caller); err != nil {
		return err
	}
	if !IsServiceCallStatus(r.Status) {
		return fmt.Errorf("unknown aethercore service receipt status %q", r.Status)
	}
	if err := ValidateHash("aethercore service receipt request hash", r.RequestHash); err != nil {
		return err
	}
	if err := validateOptionalHash("aethercore service receipt response hash", r.ResponseHash); err != nil {
		return err
	}
	if err := validateOptionalHash("aethercore service receipt proof hash", r.ProofHash); err != nil {
		return err
	}
	if !IsServicePaymentStatus(r.PaymentStatus) {
		return fmt.Errorf("unknown aethercore service receipt payment status %q", r.PaymentStatus)
	}
	if r.ProviderID != "" {
		if err := validatePolicyID("aethercore service receipt provider id", r.ProviderID); err != nil {
			return err
		}
	}
	if r.ExecutedHeight == 0 || r.AnchoredHeight == 0 {
		return errors.New("aethercore service receipt heights must be positive")
	}
	if r.ErrorCode != "" {
		if err := validatePolicyID("aethercore service receipt error code", r.ErrorCode); err != nil {
			return err
		}
	}
	if err := ValidateHash("aethercore service receipt hash", r.ReceiptHash); err != nil {
		return err
	}
	if expected := ComputeServiceCallReceiptHash(r); r.ReceiptHash != expected {
		return fmt.Errorf("aethercore service receipt hash mismatch: expected %s", expected)
	}
	return nil
}
...
func (f ServiceFinalization) Validate() error {
	if f.Height == 0 {
		return errors.New("aethercore service finalization height must be positive")
	}
	if err := ValidateHash("aethercore service finalization registry root", f.RegistryRoot); err != nil {
		return err
	}
	if err := ValidateHash("aethercore service finalization interface root", f.InterfaceRoot); err != nil {
		return err
	}
	if err := ValidateHash("aethercore service finalization receipts root", f.ServiceReceiptsRoot); err != nil {
		return err
	}
	...
	if receiptsRoot != f.ServiceReceiptsRoot {
		return errors.New("aethercore service finalization receipts root mismatch")
	}
	if err := ValidateHash("aethercore service finalization hash", f.FinalizationHash); err != nil {
		return err
	}
	if expected := ComputeServiceFinalizationHash(f); f.FinalizationHash != expected {
		return fmt.Errorf("aethercore service finalization hash mismatch: expected %s", expected)
	}
	return nil
}
...
func (m ServiceEndBlockMaintenance) Validate() error {
	if m.Height == 0 {
		return errors.New("aethercore service maintenance height must be positive")
	}
	if err := validateSortedStringSet("aethercore expired service id", m.ExpiredServiceIDs); err != nil {
		return err
	}
	if err := validateSortedStringSet("aethercore cleanup service id", m.CleanupServiceIDs); err != nil {
		return err
	}
	for i, delta := range m.ReputationDeltas {
		...
		if i > 0 && m.ReputationDeltas[i-1].ProviderID >= delta.ProviderID {
			return errors.New("aethercore provider reputation deltas must be sorted canonically")
		}
	}
	if err := ValidateHash("aethercore service maintenance hash", m.MaintenanceHash); err != nil {
		return err
	}
	if expected := ComputeServiceEndBlockMaintenanceHash(m); m.MaintenanceHash != expected {
		return fmt.Errorf("aethercore service maintenance hash mismatch: expected %s", expected)
	}
	return nil
}

func (d ProviderReputationDelta) Validate() error {
	if err := validatePolicyID("aethercore provider reputation id", d.ProviderID); err != nil {
		return err
	}
	if d.Successes == 0 && d.Failures == 0 {
		return errors.New("aethercore provider reputation delta must change score")
	}
	return nil
}
...
func (t ServiceStateTransition) Validate(state CoreState) error {
	...
	if err := ValidateHash("aethercore service STF current state root", t.CurrentStateRoot); err != nil {
		return err
	}
	if err := ValidateHash("aethercore service STF next state root", t.NextStateRoot); err != nil {
		return err
	}
	...
	if err := validateServiceStateKeys("aethercore service STF read set", t.StateReadSet); err != nil {
		return err
	}
	if err := validateServiceStateKeys("aethercore service STF write set", t.StateWriteSet); err != nil {
		return err
	}
	if len(t.ExternalCalls) > 0 {
		return errors.New("aethercore service STF must not perform external network calls")
	}
	if t.UsesWallClock {
	...
	if serviceProofRequiresMetering(t.Call.ProofRequirement) && t.ProofVerificationGas == 0 {
		return errors.New("aethercore service STF proof verification must be metered")
	}
	if len(t.DirectCrossZoneWrites) > 0 {
		return errors.New("aethercore service STF must not perform direct cross-zone writes")
	}
	return nil
}
...
func ComputeServiceCallStateScope(call ServiceCallEnvelope) string {
	...
	parts := []string{"aetheris-aek-service-state-scope-v1"}
	if len(writeSet) > 0 {
...
func ComputeServiceExecutionGroupID(scope string) string {
	return hashParts("aetheris-aek-service-execution-group-v1", scope)
}
...
func ComputeServiceInterfaceRoot(services []ServiceDescriptor) (string, error) {
	...
	parts := []string{"aetheris-aek-service-interfaces-root-v1", fmt.Sprint(len(ordered))}
	var previous string
...
func ComputeServiceProposalPlanHash(plan ServiceProposalPlan) string {
	...
	sortServiceExecutionGroups(groups)
	parts := []string{
		"aetheris-aek-service-proposal-plan-v1",
		plan.ChainID,
...
func ComputeServiceCallEnvelopeHash(call ServiceCallEnvelope) string {
	...
	call.StateWriteSet = normalizeServiceStringSet(call.StateWriteSet)
	parts := []string{
		"aetheris-aek-service-call-envelope-v1",
		call.CallID,
		call.ServiceID,
		call.Caller,
		fmt.Sprint(call.Nonce),
...
func ComputeServiceCallReceiptHash(receipt ServiceCallReceipt) string {
	return hashParts(
		"aetheris-aek-service-call-receipt-v1",
		receipt.CallID,
		receipt.ServiceID,
		receipt.MethodID,
		receipt.Caller,
...
func ComputeServiceReceiptsRoot(receipts []ServiceCallReceipt) (string, error) {
	...
	parts := []string{"aetheris-aek-service-receipts-root-v1", fmt.Sprint(len(ordered))}
	var previous string
	for _, receipt := range ordered {
		...
		if previous != "" && previous >= receipt.CallID {
			return "", errors.New("aethercore service receipts must be sorted canonically by call id")
		}
...
func ComputeServiceFinalizationHash(finalization ServiceFinalization) string {
	return hashParts(
		"aetheris-aek-service-finalization-v1",
...
func ComputeServiceEndBlockMaintenanceHash(m ServiceEndBlockMaintenance) string {
	parts := []string{
		"aetheris-aek-service-endblock-maintenance-v1",
		fmt.Sprint(m.Height),
