...
func validateZoneID(id string) error {
	...
	if text != id || text == "" {
		return errors.New("aethercore zone id is required and must not have surrounding whitespace")
	}
	if len(text) > MaxIDLength {
		return fmt.Errorf("aethercore zone id must be <= %d bytes", MaxIDLength)
	}
	for i, r := range text {
		if i == 0 && (r < 'A' || r > 'Z') {
			return errors.New("aethercore zone id must start with A-Z")
		}
		if r >= 'A' && r <= 'Z' || r >= '0' && r <= '9' || r == '_' {
			continue
		}
		return errors.New("aethercore zone id must contain only A-Z, 0-9, or underscore")
	}
	return nil
}
...
func validateCapabilities(capabilities []string) error {
	return validateCapabilitiesForField("aethercore zone capability", capabilities)
}
...
func validateModuleName(value string) error {
	return validateToken("aethercore zone module name", value, MaxScopeLength)
}
