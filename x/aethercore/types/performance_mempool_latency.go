...
func BuildMempoolSeparationPlan(height uint64, txs []MempoolAdmissionTx, params MempoolSeparationParams) (MempoolSeparationPlan, error) {
	if height == 0 {
		return MempoolSeparationPlan{}, errors.New("aethercore mempool separation height must be positive")
	}
	...
	for _, tx := range ordered {
		...
		if senderCounts[tx.Sender] > params.MaxPerSender {
			return MempoolSeparationPlan{}, errors.New("aethercore mempool sender DoS limit exceeded")
		}
		targetKey := tx.TargetZoneShardObject()
		targetCounts[targetKey]++
		if targetCounts[targetKey] > params.MaxPerTargetObject {
			return MempoolSeparationPlan{}, errors.New("aethercore mempool target object DoS limit exceeded")
		}
		laneKey := tx.LaneKey()
...
func (p MempoolSeparationParams) Validate() error {
	p = p.Normalize()
	if p.MaxPerSender == 0 {
		return errors.New("aethercore mempool max per sender must be positive")
	}
	if p.MaxPerTargetObject == 0 {
		return errors.New("aethercore mempool max per target object must be positive")
	}
	if err := ValidateShardID(p.SystemShardID); err != nil {
		return err
	}
	if err := ValidateHash("aethercore mempool params hash", p.ParamsHash); err != nil {
		return err
	}
	if p.ParamsHash != ComputeMempoolSeparationParamsHash(p) {
		return errors.New("aethercore mempool params hash mismatch")
	}
	return nil
}
...
func (tx MempoolAdmissionTx) Validate(params MempoolSeparationParams) error {
	tx = tx.Normalize()
	params = params.Normalize()
	if err := ValidateHash("aethercore mempool tx hash", tx.TxHash); err != nil {
		return err
	}
	if err := validateToken("aethercore mempool sender", tx.Sender, MaxScopeLength); err != nil {
		return err
	}
	...
	if !IsMempoolMessageClass(tx.MessageClass) {
		return fmt.Errorf("unknown aethercore mempool message class %q", tx.MessageClass)
	}
	if tx.AdmissionHeight == 0 {
		return errors.New("aethercore mempool admission height must be positive")
	}
	if tx.ExpiryHeight != 0 && tx.ExpiryHeight < tx.AdmissionHeight {
		return errors.New("aethercore mempool expiry precedes admission")
	}
	if tx.TargetObject == "" {
		return errors.New("aethercore mempool target object is required")
	}
	if tx.RouteKey == "" && tx.TargetKnown {
		return errors.New("aethercore mempool target route key is required when known")
	}
	if !tx.TargetKnown && !tx.PreResolved && tx.TargetShardID != params.SystemShardID {
		return errors.New("aethercore mempool unknown target must be pre-resolved or routed to system shard")
	}
	return nil
}
...
func (l MempoolLane) Validate(params MempoolSeparationParams) error {
	...
	if !IsMempoolMessageClass(l.MessageClass) {
		return fmt.Errorf("unknown aethercore mempool lane message class %q", l.MessageClass)
	}
	if len(l.Transactions) == 0 {
		return errors.New("aethercore mempool lane requires transactions")
	}
	for _, tx := range l.Transactions {
		...
		if tx.TargetZoneID != l.ZoneID || tx.TargetShardID != l.ShardID || tx.MessageClass != l.MessageClass {
			return errors.New("aethercore mempool lane transaction route mismatch")
		}
	}
	if err := ValidateHash("aethercore mempool lane hash", l.LaneHash); err != nil {
		return err
	}
	if l.LaneHash != ComputeMempoolLaneHash(l) {
		return errors.New("aethercore mempool lane hash mismatch")
	}
	return nil
}
...
func (p MempoolSeparationPlan) Validate() error {
	p = p.Normalize()
	if p.Height == 0 {
		return errors.New("aethercore mempool separation height must be positive")
	}
	if err := p.Params.Validate(); err != nil {
		return err
	}
	if len(p.Lanes) == 0 {
		return errors.New("aethercore mempool separation plan requires lanes")
	}
	var previous string
	for _, lane := range p.Lanes {
		...
		if previous != "" && previous >= key {
			return errors.New("aethercore mempool lanes must be sorted canonically")
		}
		previous = key
	}
	if err := ValidateHash("aethercore mempool separation plan hash", p.PlanHash); err != nil {
		return err
	}
	if p.PlanHash != ComputeMempoolSeparationPlanHash(p) {
		return errors.New("aethercore mempool separation plan hash mismatch")
	}
	return nil
}
...
func (t LatencyTarget) Validate() error {
	...
	if !IsLatencyOperationClass(t.OperationClass) {
		return fmt.Errorf("unknown aethercore latency operation class %q", t.OperationClass)
	}
	if t.TargetBlocks == 0 {
		return errors.New("aethercore latency target blocks must be positive")
	}
	if t.Description == "" {
		return errors.New("aethercore latency target description is required")
	}
	if err := ValidateHash("aethercore latency target hash", t.TargetHash); err != nil {
		return err
	}
	if t.TargetHash != ComputeLatencyTargetHash(t) {
		return errors.New("aethercore latency target hash mismatch")
	}
	return nil
}
...
func (m LatencyMetric) Validate() error {
	...
	if !IsLatencyOperationClass(m.OperationClass) {
		return fmt.Errorf("unknown aethercore latency metric operation class %q", m.OperationClass)
	}
	if m.SampleCount == 0 {
		return errors.New("aethercore latency metric sample count must be positive")
	}
	if m.Height == 0 {
		return errors.New("aethercore latency metric height must be positive")
	}
	if err := ValidateHash("aethercore latency metric hash", m.MetricHash); err != nil {
		return err
	}
	if m.MetricHash != ComputeLatencyMetricHash(m) {
		return errors.New("aethercore latency metric hash mismatch")
	}
	return nil
}
...
func (s CrossZoneMessageSLAParams) Validate() error {
	...
	if s.MaxFinalityDelayBlocks == 0 || s.MaxQueueDelayBlocks == 0 || s.MaxDeliveryBlocks == 0 {
		return errors.New("aethercore cross-zone SLA bounds must be positive")
	}
	if s.MaxDeliveryBlocks < s.MaxFinalityDelayBlocks+s.MaxQueueDelayBlocks {
		return errors.New("aethercore cross-zone SLA delivery bound is too low")
	}
	if err := ValidateHash("aethercore cross-zone SLA hash", s.ParamsHash); err != nil {
		return err
	}
	if s.ParamsHash != ComputeCrossZoneMessageSLAParamsHash(s) {
		return errors.New("aethercore cross-zone SLA hash mismatch")
	}
	return nil
}
...
func (p CongestionForwardingFeePolicy) Validate() error {
	p = p.Normalize()
	if p.BaseFeeNAET == 0 {
		return errors.New("aethercore forwarding base fee must be positive")
	}
	if p.MaxForwardingFeeNAET < p.BaseFeeNAET {
		return errors.New("aethercore forwarding max fee must cover base fee")
	}
	if p.CriticalLaneDiscountBps > 10_000 {
		return errors.New("aethercore forwarding critical discount must be <= 10000 bps")
	}
	if err := ValidateHash("aethercore forwarding fee policy hash", p.PolicyHash); err != nil {
		return err
	}
	if p.PolicyHash != ComputeCongestionForwardingFeePolicyHash(p) {
		return errors.New("aethercore forwarding fee policy hash mismatch")
	}
	return nil
}
...
func (d DeliveryPriorityItem) Validate() error {
	d = d.Normalize()
	if err := ValidateHash("aethercore delivery message id", d.MessageID); err != nil {
		return err
	}
	if !IsLatencyOperationClass(d.OperationClass) {
		return fmt.Errorf("unknown aethercore delivery operation class %q", d.OperationClass)
	}
	...
	if d.ExpiryHeight == 0 || d.EnqueuedHeight == 0 {
		return errors.New("aethercore delivery expiry and enqueue height must be positive")
	}
	if d.ExpiryHeight < d.EnqueuedHeight {
		return errors.New("aethercore delivery expiry precedes enqueue height")
	}
	if err := ValidateHash("aethercore delivery item hash", d.ItemHash); err != nil {
		return err
	}
	if d.ItemHash != ComputeDeliveryPriorityItemHash(d) {
		return errors.New("aethercore delivery item hash mismatch")
	}
	return nil
}
...
func (s LatencyStrategySpec) Validate() error {
	...
	if s.Version != LatencyStrategySpecVersion {
		return fmt.Errorf("aethercore latency strategy version must be %d", LatencyStrategySpecVersion)
	}
	...
	if len(s.Targets) != 4 {
		return errors.New("aethercore latency strategy requires all operation targets")
	}
	...
	for _, class := range []LatencyOperationClass{LatencySingleZoneLocal, LatencySameZoneCrossShard, LatencyCrossZoneAsync, LatencyContractPromiseResolve} {
		if _, found := seen[class]; !found {
			return fmt.Errorf("aethercore latency strategy missing target %s", class)
		}
	}
	for _, item := range []struct {
		...
		{"aethercore latency metrics root", s.MetricsRoot},
		{"aethercore latency delivery root", s.DeliveryRoot},
		{"aethercore latency strategy root", s.Root},
	} {
	...
	if s.Root != ComputeLatencyStrategySpecRoot(s) {
		return errors.New("aethercore latency strategy root mismatch")
	}
	return nil
}
...
func ComputeMempoolSeparationParamsHash(params MempoolSeparationParams) string {
	params = params.Normalize()
	return hashParts("aetheris-mempool-separation-params-v1", fmt.Sprint(params.MaxPerSender), fmt.Sprint(params.MaxPerTargetObject), string(params.SystemShardID))
}

func ComputeMempoolAdmissionTxHash(tx MempoolAdmissionTx) string {
	tx = tx.Normalize()
	return hashParts(
		"aetheris-mempool-admission-tx-v1",
		tx.TxHash,
		tx.Sender,
...
func ComputeMempoolLaneHash(lane MempoolLane) string {
	lane = lane.Normalize()
	parts := []string{"aetheris-mempool-lane-v1", string(lane.ZoneID), string(lane.ShardID), string(lane.MessageClass), fmt.Sprint(len(lane.Transactions))}
...
func ComputeMempoolSeparationPlanHash(plan MempoolSeparationPlan) string {
	plan = plan.Normalize()
	parts := []string{"aetheris-mempool-separation-plan-v1", fmt.Sprint(plan.Height), plan.Params.ParamsHash, fmt.Sprint(len(plan.Lanes))}
...
func ComputeLatencyTargetHash(target LatencyTarget) string {
	...
	return hashParts("aetheris-latency-target-v1", string(target.OperationClass), fmt.Sprint(target.TargetBlocks), target.Description)
}

func ComputeLatencyMetricHash(metric LatencyMetric) string {
	metric = metric.Normalize()
	return hashParts("aetheris-latency-metric-v1", string(metric.OperationClass), fmt.Sprint(metric.ObservedBlocks), fmt.Sprint(metric.SampleCount), fmt.Sprint(metric.Height))
}

func ComputeCrossZoneMessageSLAParamsHash(sla CrossZoneMessageSLAParams) string {
	sla = sla.Normalize()
	return hashParts("aetheris-cross-zone-message-sla-v1", fmt.Sprint(sla.MaxFinalityDelayBlocks), fmt.Sprint(sla.MaxQueueDelayBlocks), fmt.Sprint(sla.MaxDeliveryBlocks))
}

func ComputeCongestionForwardingFeePolicyHash(policy CongestionForwardingFeePolicy) string {
	policy = policy.Normalize()
	return hashParts("aetheris-congestion-forwarding-fee-policy-v1", fmt.Sprint(policy.BaseFeeNAET), fmt.Sprint(policy.CongestionWeightBps), fmt.Sprint(policy.NearExpiryPremiumBps), fmt.Sprint(policy.MaxForwardingFeeNAET), fmt.Sprint(policy.CriticalLaneDiscountBps))
}

func ComputeDeliveryPriorityItemHash(item DeliveryPriorityItem) string {
	item = item.Normalize()
	return hashParts("aetheris-delivery-priority-item-v1", item.MessageID, string(item.OperationClass), string(item.DestinationZone), string(item.DestinationShard), fmt.Sprint(item.FeeNAET), fmt.Sprint(item.CongestionScore), fmt.Sprint(item.ExpiryHeight), fmt.Sprint(item.EnqueuedHeight), fmt.Sprint(item.Critical))
}

func ComputeLatencyMetricsRoot(metrics []LatencyMetric) string {
	ordered := normalizeLatencyMetrics(metrics)
	parts := []string{"aetheris-latency-metrics-root-v1", fmt.Sprint(len(ordered))}
...
func ComputeDeliveryPriorityQueueRoot(queue []DeliveryPriorityItem) string {
	...
	parts := []string{"aetheris-delivery-priority-queue-root-v1", fmt.Sprint(len(ordered))}
...
func ComputeLatencyStrategySpecRoot(spec LatencyStrategySpec) string {
	spec = spec.Normalize()
	parts := []string{"aetheris-latency-strategy-spec-v1", fmt.Sprint(spec.Version), spec.SLA.ParamsHash, spec.FeePolicy.PolicyHash, spec.MetricsRoot, spec.DeliveryRoot}
