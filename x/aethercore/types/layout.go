...
func (d ShardDescriptor) Validate() error {
	...
	if d.KeyPrefix != "" {
		if err := validateToken("aethercore shard key prefix", d.KeyPrefix, MaxScopeLength); err != nil {
			return err
		}
	}
	if d.HashRangeEnd < d.HashRangeStart {
		return errors.New("aethercore shard hash range end must not precede start")
	}
...
func (l ShardLayout) ValidateFormat() error {
	...
	if !IsShardAssignmentMode(mode) {
		return fmt.Errorf("unknown aethercore shard assignment mode %q", mode)
	}
	...
	if l.SystemShardID != "" && !l.HasActiveShard(l.SystemShardID) {
		return errors.New("aethercore shard layout system shard must be active")
	}
...
func RouteKeyToShard(layout ShardLayout, input ShardRoutingInput) (ShardRoute, error) {
	...
	if input.ZoneID != layout.ZoneID {
		return ShardRoute{}, errors.New("aethercore shard route zone mismatch")
	}
	if input.ShardLayoutEpoch != layout.LayoutEpoch {
		return ShardRoute{}, errors.New("aethercore shard route layout epoch mismatch")
	}
	if err := validateToken("aethercore shard route state key", input.StateKey, MaxScopeLength); err != nil {
		return ShardRoute{}, err
	}
	...
	if !IsShardAssignmentMode(mode) {
		return ShardRoute{}, fmt.Errorf("unknown aethercore shard assignment mode %q", mode)
	}
	if input.PlacementOverride != "" {
		if !layout.HasActiveShard(input.PlacementOverride) {
			return ShardRoute{}, errors.New("aethercore shard route placement override is not active")
		}
...
func (r ShardRoute) ValidateHash() error {
	...
	if r.LayoutEpoch == 0 {
		return errors.New("aethercore shard route layout epoch must be positive")
	}
	if !IsShardAssignmentMode(r.AssignmentMode) {
		return fmt.Errorf("unknown aethercore shard route mode %q", r.AssignmentMode)
	}
	if err := ValidateShardID(r.ShardID); err != nil {
		return err
	}
	if r.ShardCount == 0 {
		return errors.New("aethercore shard route shard count must be positive")
	}
	if err := validateToken("aethercore shard route state key", r.StateKey, MaxScopeLength); err != nil {
		return err
	}
	...
	if err := ValidateHash("aethercore shard route hash", r.RouteHash); err != nil {
		return err
	}
	if r.RouteHash != ComputeShardRouteHash(r) {
		return errors.New("aethercore shard route hash mismatch")
	}
	return nil
}
...
func NewShardMetrics(metrics ShardMetrics) (ShardMetrics, error) {
	if metrics.MetricsHash != "" {
		return ShardMetrics{}, errors.New("aethercore shard metrics hash must be empty before construction")
	}
...
func (m ShardMetrics) ValidateFormat() error {
	...
	if m.Height == 0 {
		return errors.New("aethercore shard metrics height must be positive")
	}
	if m.MetricsHash != "" {
		return ValidateHash("aethercore shard metrics hash", m.MetricsHash)
	}
	return nil
}
...
func (m ShardMetrics) ValidateHash() error {
	...
	if m.MetricsHash != ComputeShardMetricsHash(m) {
		return errors.New("aethercore shard metrics hash mismatch")
	}
	return nil
}
...
func validateShardPlacementOverrides(overrides []ShardPlacementOverride, layout ShardLayout) error {
	...
	for i, override := range ordered {
		if err := validateToken("aethercore shard placement object key", override.ObjectKey, MaxScopeLength); err != nil {
			return err
		}
		...
		if !layout.HasActiveShard(override.ShardID) {
			return errors.New("aethercore shard placement override shard must be active")
		}
		if i > 0 && previous >= override.ObjectKey {
			return errors.New("aethercore shard placement overrides must be sorted canonically")
		}
...
func validateReadOnlyReplicatedKeys(keys []string) error {
	...
	for i, key := range ordered {
		if err := validateToken("aethercore read-only replicated key", key, MaxScopeLength); err != nil {
			return err
		}
		if i > 0 && previous >= key {
			return errors.New("aethercore read-only replicated keys must be sorted canonically")
		}
		previous = key
	}
	return nil
...
func routeKeyPrefixShard(layout ShardLayout, stateKey string) (ShardID, error) {
	...
	if selected.ShardID == "" {
		return "", errors.New("aethercore shard prefix route has no matching shard")
	}
...
func routeConsistentHashShard(layout ShardLayout, stateKey string) (ShardID, error) {
	...
	if len(active) == 0 {
		return "", errors.New("aethercore shard route requires available shards")
	}
	routeHash := hashParts("aetheris-aek-route-key-v1", string(layout.ZoneID), fmt.Sprint(layout.LayoutEpoch), layout.RoutingSeedHash, stateKey)
