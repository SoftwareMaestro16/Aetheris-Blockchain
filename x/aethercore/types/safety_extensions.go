...
func (s ExtendedSafetyRulesSpec) ValidateFormat() error {
	...
	if s.Version != ExtendedSafetyRulesSpecVersion {
		return fmt.Errorf("aethercore extended safety spec version must be %d", ExtendedSafetyRulesSpecVersion)
	}
	if len(s.ShardSafety) == 0 || len(s.VMSafety) == 0 || len(s.ProofSafety) == 0 {
		return errors.New("aethercore extended safety spec requires shard, VM, and proof rules")
	}
...
func (s ExtendedSafetyRulesSpec) Validate() error {
	...
	for _, item := range []struct {
		...
		{"aethercore shard safety root", s.ShardSafetyRoot, ComputeShardSafetyRulesRoot(s.ShardSafety)},
		{"aethercore VM safety root", s.VMSafetyRoot, ComputeVMSafetyRulesRoot(s.VMSafety)},
		{"aethercore proof safety root", s.ProofSafetyRoot, ComputeProofSafetyRulesRoot(s.ProofSafety)},
		{"aethercore extended safety root", s.Root, ComputeExtendedSafetyRulesSpecRoot(s)},
	} {
...
func ValidateShardSafetyEvidence(e ShardSafetyEvidence) error {
	...
	if e.SourceLayoutEpoch == 0 || e.TargetLayoutEpoch == 0 {
		return errors.New("aethercore shard safety layout epochs must be positive")
	}
	if e.TargetLayoutEpoch <= e.SourceLayoutEpoch {
		return errors.New("aethercore shard safety target epoch must be future")
	}
	if e.DecisionHeight == 0 || e.ActivationHeight == 0 || e.ActivationHeight <= e.DecisionHeight {
		return errors.New("aethercore shard safety activation height must be after decision height")
	}
	if e.DeliveryEpoch < e.TargetLayoutEpoch {
		return errors.New("aethercore shard safety delivery epoch must include target layout epoch")
	}
	if e.ProofHorizonUntil < e.ActivationHeight {
		return errors.New("aethercore shard safety old layout proof horizon is too short")
	}
	for _, item := range []struct {
		name  string
		value string
	}{
		{"aethercore shard safety decision hash", e.DecisionHash},
		{"aethercore shard safety committed metrics root", e.CommittedMetricsRoot},
		{"aethercore shard safety migration root", e.MigrationRoot},
		{"aethercore shard safety old layout hash", e.OldLayoutHash},
		{"aethercore shard safety evidence hash", e.EvidenceHash},
	} {
		if err := ValidateHash(item.name, item.value); err != nil {
			return err
		}
	...
	if e.EvidenceHash != ComputeShardSafetyEvidenceHash(e) {
		return errors.New("aethercore shard safety evidence hash mismatch")
	}
	return nil
}
...
func ValidateVMSafetyEvidence(e VMSafetyEvidence) error {
	...
	if err := ValidateHash("aethercore VM safety gas table hash", e.GasTableHash); err != nil {
		return err
	}
	if e.GasLimit == 0 {
		return errors.New("aethercore VM safety gas metering is required")
	}
	if e.GasUsed > e.GasLimit {
		return errors.New("aethercore VM safety gas used exceeds gas limit")
	}
	if e.MaxStorageIterationItems == 0 {
		return errors.New("aethercore VM safety storage iteration must be bounded")
	}
	if e.ProofVerificationGas == 0 {
		return errors.New("aethercore VM safety proof verification gas is required")
	}
	if e.CreatedMessageCount > 0 && e.ForwardingFeeReserved == 0 {
		return errors.New("aethercore VM safety message creation requires reserved forwarding fee")
	}
	if e.SynchronousRemoteMutation {
		return errors.New("aethercore VM safety forbids synchronous remote zone mutation")
	}
	if e.PromiseTimeoutHeight != 0 && e.PromiseTimeoutHeight <= e.ConsensusHeight {
		return errors.New("aethercore VM safety promise timeout must be a future consensus height")
	}
	if err := ValidateHash("aethercore VM safety evidence hash", e.EvidenceHash); err != nil {
		return err
	}
	if e.EvidenceHash != ComputeVMSafetyEvidenceHash(e) {
		return errors.New("aethercore VM safety evidence hash mismatch")
	}
	return nil
}
...
func ValidateProofSafetyEvidence(e ProofSafetyEvidence) error {
	...
	if e.ProofVersion != UniversalProofVersionV1 {
		return errors.New("aethercore proof safety unsupported proof version")
	}
	if e.TrustedHeaderHeight == 0 || e.ProofHeight == 0 || e.ProofHeight != e.TrustedHeaderHeight {
		return errors.New("aethercore proof safety proof height must bind to trusted header height")
	}
	...
	if e.RootType == "" {
		return errors.New("aethercore proof safety root type is required")
	}
	if len(e.ObjectKey) == 0 {
		return errors.New("aethercore proof safety object key is required")
	}
	if e.NonExistenceProof && len(e.AbsenceMarker) == 0 {
		return errors.New("aethercore proof safety non-existence proof requires explicit marker")
	}
	if err := ValidateHash("aethercore proof safety evidence hash", e.EvidenceHash); err != nil {
		return err
	}
	if e.EvidenceHash != ComputeProofSafetyEvidenceHash(e) {
		return errors.New("aethercore proof safety evidence hash mismatch")
	}
	return nil
}
...
func (r ShardSafetyRule) Validate() error {
	...
	if !IsShardSafetyRuleID(r.RuleID) {
		return fmt.Errorf("unknown aethercore shard safety rule %q", r.RuleID)
	}
	if r.Rule == "" || r.Enforcement == "" || r.Evidence == "" {
		return errors.New("aethercore shard safety rule requires rule, enforcement, and evidence")
	}
	if err := ValidateHash("aethercore shard safety rule hash", r.DescriptorHash); err != nil {
		return err
	}
	if r.DescriptorHash != ComputeShardSafetyRuleHash(r) {
		return errors.New("aethercore shard safety rule hash mismatch")
	}
	return nil
}
...
func (r VMSafetyRule) Validate() error {
	...
	if !IsVMSafetyRuleID(r.RuleID) {
		return fmt.Errorf("unknown aethercore VM safety rule %q", r.RuleID)
	}
	if r.Rule == "" || r.Enforcement == "" || r.Evidence == "" {
		return errors.New("aethercore VM safety rule requires rule, enforcement, and evidence")
	}
	if err := ValidateHash("aethercore VM safety rule hash", r.DescriptorHash); err != nil {
		return err
	}
	if r.DescriptorHash != ComputeVMSafetyRuleHash(r) {
		return errors.New("aethercore VM safety rule hash mismatch")
	}
	return nil
}
...
func (r ProofSafetyRule) Validate() error {
	...
	if !IsProofSafetyRuleID(r.RuleID) {
		return fmt.Errorf("unknown aethercore proof safety rule %q", r.RuleID)
	}
	if r.Rule == "" || r.Enforcement == "" || r.Evidence == "" {
		return errors.New("aethercore proof safety rule requires rule, enforcement, and evidence")
	}
	if err := ValidateHash("aethercore proof safety rule hash", r.DescriptorHash); err != nil {
		return err
	}
	if r.DescriptorHash != ComputeProofSafetyRuleHash(r) {
		return errors.New("aethercore proof safety rule hash mismatch")
	}
	return nil
}
...
func ComputeShardSafetyRuleHash(rule ShardSafetyRule) string {
	rule = rule.Normalize()
	return hashParts("aetheris-shard-safety-rule-v1", string(rule.RuleID), rule.Rule, rule.Enforcement, rule.Evidence)
}

func ComputeVMSafetyRuleHash(rule VMSafetyRule) string {
	rule = rule.Normalize()
	return hashParts("aetheris-vm-safety-rule-v1", string(rule.RuleID), rule.Rule, rule.Enforcement, rule.Evidence)
}

func ComputeProofSafetyRuleHash(rule ProofSafetyRule) string {
	rule = rule.Normalize()
	return hashParts("aetheris-proof-safety-rule-v1", string(rule.RuleID), rule.Rule, rule.Enforcement, rule.Evidence)
}

func ComputeShardSafetyRulesRoot(rules []ShardSafetyRule) string {
	ordered := normalizeShardSafetyRules(rules)
	parts := []string{"aetheris-shard-safety-rules-root-v1", fmt.Sprintf("%020d", ExtendedSafetyRulesSpecVersion)}
...
func ComputeVMSafetyRulesRoot(rules []VMSafetyRule) string {
	...
	parts := []string{"aetheris-vm-safety-rules-root-v1", fmt.Sprintf("%020d", ExtendedSafetyRulesSpecVersion)}
	for _, rule := range ordered {
		parts = append(parts, string(rule.RuleID), rule.DescriptorHash)
	}
	return hashParts(parts...)
...
func ComputeProofSafetyRulesRoot(rules []ProofSafetyRule) string {
	...
	parts := []string{"aetheris-proof-safety-rules-root-v1", fmt.Sprintf("%020d", ExtendedSafetyRulesSpecVersion)}
	for _, rule := range ordered {
		parts = append(parts, string(rule.RuleID), rule.DescriptorHash)
	}
	return hashParts(parts...)
...
func ComputeExtendedSafetyRulesSpecRoot(spec ExtendedSafetyRulesSpec) string {
	spec = spec.Normalize()
	return hashParts("aetheris-extended-safety-rules-spec-v1", fmt.Sprintf("%020d", spec.Version), spec.ShardSafetyRoot, spec.VMSafetyRoot, spec.ProofSafetyRoot)
}

func ComputeShardSafetyEvidenceHash(e ShardSafetyEvidence) string {
	e = normalizeShardSafetyEvidence(e)
	e.EvidenceHash = ""
	return hashParts("aetheris-shard-safety-evidence-v1", fmt.Sprint(e.SourceLayoutEpoch), fmt.Sprint(e.TargetLayoutEpoch), fmt.Sprint(e.ActivationHeight), fmt.Sprint(e.DecisionHeight), e.DecisionHash, e.CommittedMetricsRoot, fmt.Sprint(e.DeliveryEpoch), e.MigrationRoot, e.OldLayoutHash, fmt.Sprint(e.ProofHorizonUntil))
}

func ComputeVMSafetyEvidenceHash(e VMSafetyEvidence) string {
	e = normalizeVMSafetyEvidence(e)
	e.EvidenceHash = ""
	return hashParts("aetheris-vm-safety-evidence-v1", e.GasTableHash, fmt.Sprint(e.GasLimit), fmt.Sprint(e.GasUsed), fmt.Sprint(e.MaxStorageIterationItems), fmt.Sprint(e.ProofVerificationGas), fmt.Sprint(e.ForwardingFeeReserved), fmt.Sprint(e.CreatedMessageCount), fmt.Sprint(e.SynchronousRemoteMutation), fmt.Sprint(e.PromiseTimeoutHeight), fmt.Sprint(e.ConsensusHeight))
}

func ComputeProofSafetyEvidenceHash(e ProofSafetyEvidence) string {
	e = normalizeProofSafetyEvidence(e)
	e.EvidenceHash = ""
	return hashParts("aetheris-proof-safety-evidence-v1", fmt.Sprint(e.ProofVersion), fmt.Sprint(e.TrustedHeaderHeight), fmt.Sprint(e.ProofHeight), string(e.ZoneID), string(e.ShardID), string(e.RootType), string(e.ObjectKey), fmt.Sprint(e.NonExistenceProof), string(e.AbsenceMarker))
}
...
func validateShardSafetyRules(rules []ShardSafetyRule) error {
	...
	for i, rule := range rules {
		...
		if _, found := seen[rule.RuleID]; found {
			return fmt.Errorf("duplicate aethercore shard safety rule %s", rule.RuleID)
		}
		seen[rule.RuleID] = struct{}{}
		if i > 0 && previous >= rule.RuleID {
			return errors.New("aethercore shard safety rules must be sorted canonically")
		}
...
func validateVMSafetyRules(rules []VMSafetyRule) error {
	...
	for i, rule := range rules {
		...
		if _, found := seen[rule.RuleID]; found {
			return fmt.Errorf("duplicate aethercore VM safety rule %s", rule.RuleID)
		}
		seen[rule.RuleID] = struct{}{}
		if i > 0 && previous >= rule.RuleID {
			return errors.New("aethercore VM safety rules must be sorted canonically")
		}
		previous = rule.RuleID
	}
	return nil
...
func validateProofSafetyRules(rules []ProofSafetyRule) error {
	...
	for i, rule := range rules {
		...
		if _, found := seen[rule.RuleID]; found {
			return fmt.Errorf("duplicate aethercore proof safety rule %s", rule.RuleID)
		}
		seen[rule.RuleID] = struct{}{}
		if i > 0 && previous >= rule.RuleID {
			return errors.New("aethercore proof safety rules must be sorted canonically")
		}
		previous = rule.RuleID
	}
	return nil
...
func requireShardSafetyCoverage(rules []ShardSafetyRule) error {
	...
	for _, ruleID := range []ShardSafetyRuleID{ShardSafetyEpochBoundary, ShardSafetyReproducibleDecision, ShardSafetyDeliveryEpoch, ShardSafetyMigrationRoot, ShardSafetyProofHorizon} {
		if _, found := seen[ruleID]; !found {
			return fmt.Errorf("aethercore extended safety missing shard rule %s", ruleID)
		}
	}
	return nil
}
...
func requireVMSafetyCoverage(rules []VMSafetyRule) error {
	...
	for _, ruleID := range []VMSafetyRuleID{VMSafetyGasMetering, VMSafetyBoundedIteration, VMSafetyMeteredProofs, VMSafetyForwardingFeeReserve, VMSafetyNoRemoteMutation, VMSafetyDeterministicTimeout} {
		if _, found := seen[ruleID]; !found {
			return fmt.Errorf("aethercore extended safety missing VM rule %s", ruleID)
		}
	}
	return nil
}
...
func requireProofSafetyCoverage(rules []ProofSafetyRule) error {
	...
	for _, ruleID := range []ProofSafetyRuleID{ProofSafetyTrustedHeight, ProofSafetyZoneShardIDs, ProofSafetyObjectKeyRootType, ProofSafetyExplicitAbsence, ProofSafetySupportedVersion} {
		if _, found := seen[ruleID]; !found {
			return fmt.Errorf("aethercore extended safety missing proof rule %s", ruleID)
		}
	}
	return nil
}
