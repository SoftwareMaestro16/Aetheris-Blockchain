...
func (definition OffChainServiceDefinition) Validate() error {
	if err := validatePolicyID("aethercore off-chain service id", definition.ServiceID); err != nil {
		return err
	}
	if err := addressing.ValidateAuthorityAddress("aethercore off-chain service owner", definition.Owner); err != nil {
		return err
	}
	if err := ValidateZoneID(definition.ZoneID); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore off-chain service endpoint", definition.Endpoint); err != nil {
		return err
	}
	if !IsOffChainEndpointType(definition.EndpointType) {
		return fmt.Errorf("unknown aethercore off-chain endpoint type %q", definition.EndpointType)
	}
	if err := validatePolicyID("aethercore off-chain provider key", definition.ProviderKey); err != nil {
		return err
	}
	if !IsOffChainRequestSigningPolicy(definition.RequestSigningPolicy) {
		return fmt.Errorf("unknown aethercore off-chain request signing policy %q", definition.RequestSigningPolicy)
	}
	if !IsOffChainResponseSigningPolicy(definition.ResponseSigningPolicy) {
		return fmt.Errorf("unknown aethercore off-chain response signing policy %q", definition.ResponseSigningPolicy)
	}
	if !IsOffChainProofAnchorPolicy(definition.ProofAnchorPolicy) {
		return fmt.Errorf("unknown aethercore off-chain proof anchor policy %q", definition.ProofAnchorPolicy)
	}
	if !IsOffChainAvailabilityPolicy(definition.AvailabilityPolicy) {
		return fmt.Errorf("unknown aethercore off-chain availability policy %q", definition.AvailabilityPolicy)
	}
	if definition.ResultExpiry == 0 {
		return errors.New("aethercore off-chain service result expiry must be positive")
	}
	if err := validatePolicyID("aethercore off-chain interface id", definition.InterfaceID); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore off-chain endpoint key", definition.EndpointKey); err != nil {
		return err
	}
	if definition.Version == 0 {
		return errors.New("aethercore off-chain service version must be positive")
	}
	if err := ValidateHash("aethercore off-chain availability hash", definition.AvailabilityHash); err != nil {
		return err
	}
	if err := validateOptionalHash("aethercore off-chain provider root", definition.ProviderRoot); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore off-chain payment denom", definition.PaymentDenom); err != nil {
		return err
	}
	if err := validateAmountString("aethercore off-chain payment amount", definition.PaymentAmount); err != nil {
		return err
	}
	if definition.CreatedHeight == 0 {
		return errors.New("aethercore off-chain service created height must be positive")
	}
	if definition.UpdatedHeight < definition.CreatedHeight {
		return errors.New("aethercore off-chain service updated height must not precede created height")
	}
	if definition.ExpiryHeight != 0 && definition.ExpiryHeight <= definition.UpdatedHeight {
		return errors.New("aethercore off-chain service expiry height must exceed updated height")
	}
	if err := validateOptionalHash("aethercore off-chain metadata hash", definition.MetadataHash); err != nil {
		return err
	}
...
func (method OffChainServiceMethod) Validate() error {
	if err := validatePolicyID("aethercore off-chain method id", method.MethodID); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore off-chain method name", method.Name); err != nil {
		return err
	}
	if err := ValidateHash("aethercore off-chain method input schema hash", method.InputSchemaHash); err != nil {
		return err
	}
	if err := ValidateHash("aethercore off-chain method output schema hash", method.OutputSchemaHash); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore off-chain method payment model", method.RequiredPaymentModel); err != nil {
		return err
	}
	if !IsServiceVerificationModel(method.VerificationModel) {
		return fmt.Errorf("unknown aethercore off-chain method verification model %q", method.VerificationModel)
	}
	if method.VerificationModel == ServiceVerificationConsensusReceipt {
		return errors.New("aethercore off-chain method cannot require consensus receipt verification")
	}
	if method.TimeoutHeightDelta == 0 {
		return errors.New("aethercore off-chain method timeout must be positive")
	}
	if !IsServiceFailureBehavior(method.FailurePolicy) {
		return fmt.Errorf("unknown aethercore off-chain method failure policy %q", method.FailurePolicy)
	}
	return nil
}
...
func (schema OffChainServiceDescriptorSchema) Validate() error {
	...
	if schema.Descriptor.ServiceType != ServiceTypeOffChain {
		return errors.New("aethercore off-chain descriptor schema requires off-chain service descriptor")
	}
	if schema.Descriptor.Interface.InterfaceHash == "" {
		return errors.New("aethercore off-chain descriptor schema requires interface hash")
	}
	if err := schema.Endpoint.Validate(); err != nil {
		return err
	}
	if schema.Endpoint.Endpoint != schema.Descriptor.Execution.Endpoint {
		return errors.New("aethercore off-chain endpoint metadata mismatch")
	}
	if !IsOffChainRequestSigningPolicy(schema.RequestSigningPolicy) {
		return fmt.Errorf("unknown aethercore off-chain request signing policy %q", schema.RequestSigningPolicy)
	}
	if !IsOffChainResponseSigningPolicy(schema.ResponseSigningPolicy) {
		return fmt.Errorf("unknown aethercore off-chain response signing policy %q", schema.ResponseSigningPolicy)
	}
	if !IsOffChainProofAnchorPolicy(schema.ProofAnchorPolicy) {
		return fmt.Errorf("unknown aethercore off-chain proof anchor policy %q", schema.ProofAnchorPolicy)
	}
	if !IsOffChainAvailabilityPolicy(schema.AvailabilityPolicy) {
		return fmt.Errorf("unknown aethercore off-chain availability policy %q", schema.AvailabilityPolicy)
	}
	if schema.ResultExpiry == 0 {
		return errors.New("aethercore off-chain descriptor schema requires result expiry")
	}
	if err := ValidateHash("aethercore off-chain descriptor schema hash", schema.SchemaHash); err != nil {
		return err
	}
	if expected := ComputeOffChainServiceDescriptorSchemaHash(schema); schema.SchemaHash != expected {
		return fmt.Errorf("aethercore off-chain descriptor schema hash mismatch: expected %s", expected)
	}
	return nil
}

func (metadata OffChainEndpointMetadata) Validate() error {
	if err := validatePolicyID("aethercore off-chain endpoint", metadata.Endpoint); err != nil {
		return err
	}
	if !IsOffChainEndpointType(metadata.EndpointType) {
		return fmt.Errorf("unknown aethercore off-chain endpoint type %q", metadata.EndpointType)
	}
	if err := validatePolicyID("aethercore off-chain provider key", metadata.ProviderKey); err != nil {
		return err
	}
	if metadata.ExpiryHeight == 0 {
		return errors.New("aethercore off-chain endpoint expiry must be positive")
	}
	return validateOptionalHash("aethercore off-chain endpoint metadata hash", metadata.MetadataHash)
}
...
func (ad OffChainSignedAdvertisement) ValidateForSchema(schema OffChainServiceDescriptorSchema) error {
	...
	if ad.ServiceID != schema.Descriptor.ServiceID || ad.Owner != schema.Descriptor.Owner ||
		...
		return errors.New("aethercore off-chain signed advertisement does not match descriptor schema")
	}
	return ad.Validate()
}

func (ad OffChainSignedAdvertisement) Validate() error {
	if err := validatePolicyID("aethercore off-chain advertisement service id", ad.ServiceID); err != nil {
		return err
	}
	if err := addressing.ValidateAuthorityAddress("aethercore off-chain advertisement owner", ad.Owner); err != nil {
		return err
	}
	if err := ValidateHash("aethercore off-chain advertisement interface hash", ad.InterfaceHash); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore off-chain advertisement endpoint", ad.Endpoint); err != nil {
		return err
	}
	if !IsOffChainEndpointType(ad.EndpointType) {
		return fmt.Errorf("unknown aethercore off-chain endpoint type %q", ad.EndpointType)
	}
	if err := validatePolicyID("aethercore off-chain advertisement provider key", ad.ProviderKey); err != nil {
		return err
	}
	if ad.ExpiryHeight == 0 {
		return errors.New("aethercore off-chain advertisement expiry must be positive")
	}
	if ad.Signer != ad.Owner && ad.Signer != ad.ProviderKey {
		return errors.New("aethercore off-chain advertisement must be signed by owner or provider")
	}
	if err := ValidateHash("aethercore off-chain advertisement hash", ad.AdvertisementHash); err != nil {
		return err
	}
	if expected := ComputeOffChainAdvertisementHash(ad); ad.AdvertisementHash != expected {
		return fmt.Errorf("aethercore off-chain advertisement hash mismatch: expected %s", expected)
	}
	if err := ValidateHash("aethercore off-chain advertisement signature", ad.SignatureHash); err != nil {
		return err
	}
	if expected := ComputeOffChainAdvertisementSignatureHash(ad, ad.Signer); ad.SignatureHash != expected {
		return fmt.Errorf("aethercore off-chain advertisement signature mismatch: expected %s", expected)
	}
	return nil
}
...
func (signed OffChainSignedRequest) Validate() error {
	...
	if signed.Signer != request.Caller {
		return errors.New("aethercore off-chain request must be signed by caller")
	}
	if err := ValidateHash("aethercore off-chain request signature", signed.SignatureHash); err != nil {
		return err
	}
	if expected := ComputeOffChainRequestSignatureHash(request, signed.Signer); signed.SignatureHash != expected {
		return fmt.Errorf("aethercore off-chain request signature mismatch: expected %s", expected)
	}
	return nil
}

func (request OffChainServiceRequest) Validate() error {
	if err := validateOptionalHash("aethercore off-chain request call id", request.CallID); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore off-chain request service id", request.ServiceID); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore off-chain request method id", request.MethodID); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore off-chain request caller", request.Caller); err != nil {
		return err
	}
	if request.Nonce == 0 && request.IdempotencyKey == "" {
		return errors.New("aethercore off-chain request requires replay-safe nonce or idempotency key")
	}
	if request.IdempotencyKey != "" {
		if err := validatePolicyID("aethercore off-chain request idempotency key", request.IdempotencyKey); err != nil {
			return err
		}
	}
	if err := ValidateHash("aethercore off-chain request payload hash", request.PayloadHash); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore off-chain request provider key", request.ProviderKey); err != nil {
		return err
	}
	if request.DeadlineHeight == 0 {
		return errors.New("aethercore off-chain request deadline must be positive")
	}
	if err := ValidateHash("aethercore off-chain request hash", request.RequestHash); err != nil {
		return err
	}
	if expected := ComputeOffChainServiceRequestHash(request); request.RequestHash != expected {
		return fmt.Errorf("aethercore off-chain request hash mismatch: expected %s", expected)
	}
	return nil
}
...
func (signed OffChainSignedResponse) Validate() error {
	...
	if signed.Signer != response.ProviderKey {
		return errors.New("aethercore off-chain response must be signed by provider")
	}
	if (response.SettlementUse || response.ChallengeUse) && signed.SignatureHash == "" {
		return errors.New("aethercore off-chain settlement or challenge response requires signature")
	}
	if err := ValidateHash("aethercore off-chain response signature", signed.SignatureHash); err != nil {
		return err
	}
	if expected := ComputeOffChainResponseSignatureHash(response, signed.Signer); signed.SignatureHash != expected {
		return fmt.Errorf("aethercore off-chain response signature mismatch: expected %s", expected)
	}
	return nil
}

func (response OffChainServiceResponse) Validate() error {
	if err := validateOptionalHash("aethercore off-chain response call id", response.CallID); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore off-chain response service id", response.ServiceID); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore off-chain response method id", response.MethodID); err != nil {
		return err
	}
	if err := ValidateHash("aethercore off-chain response request hash", response.RequestHash); err != nil {
		return err
	}
	if err := ValidateHash("aethercore off-chain response hash", response.ResponseHash); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore off-chain response provider key", response.ProviderKey); err != nil {
		return err
	}
	if response.Height == 0 {
		return errors.New("aethercore off-chain response height must be positive")
	}
	if response.ResultExpiryHeight <= response.Height {
		return errors.New("aethercore off-chain response expiry must exceed response height")
	}
	return nil
}
...
func (anchor OffChainReceiptAnchorMessage) Validate() error {
	if err := validatePolicyID("aethercore off-chain anchor service id", anchor.ServiceID); err != nil {
		return err
	}
	if err := validateOptionalHash("aethercore off-chain anchor call id", anchor.CallID); err != nil {
		return err
	}
	if err := ValidateHash("aethercore off-chain anchor request hash", anchor.RequestHash); err != nil {
		return err
	}
	if err := ValidateHash("aethercore off-chain anchor response hash", anchor.ResponseHash); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore off-chain anchor provider key", anchor.ProviderKey); err != nil {
		return err
	}
	if anchor.Height == 0 {
		return errors.New("aethercore off-chain anchor height must be positive")
	}
	if err := ValidateHash("aethercore off-chain anchor proof hash", anchor.ProofHash); err != nil {
		return err
	}
	if expected := ComputeOffChainReceiptAnchorProofHash(anchor); anchor.ProofHash != expected {
		return fmt.Errorf("aethercore off-chain anchor proof hash mismatch: expected %s", expected)
	}
	if err := ValidateHash("aethercore off-chain anchor hash", anchor.AnchorHash); err != nil {
		return err
	}
	if expected := ComputeOffChainReceiptAnchorHash(anchor); anchor.AnchorHash != expected {
		return fmt.Errorf("aethercore off-chain anchor hash mismatch: expected %s", expected)
	}
	return nil
}
...
func (renewal OffChainEndpointRenewal) ValidateForSchema(schema OffChainServiceDescriptorSchema) error {
	...
	if renewal.ServiceID != schema.Descriptor.ServiceID || renewal.EndpointType != schema.Endpoint.EndpointType ||
		...
		return errors.New("aethercore off-chain endpoint renewal does not match descriptor schema")
	}
	if renewal.ExpiryHeight <= schema.Endpoint.ExpiryHeight {
		return errors.New("aethercore off-chain endpoint renewal must extend endpoint expiry")
	}
	return renewal.Validate()
}

func (renewal OffChainEndpointRenewal) Validate() error {
	if err := validatePolicyID("aethercore off-chain renewal service id", renewal.ServiceID); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore off-chain renewal endpoint", renewal.Endpoint); err != nil {
		return err
	}
	if !IsOffChainEndpointType(renewal.EndpointType) {
		return fmt.Errorf("unknown aethercore off-chain endpoint type %q", renewal.EndpointType)
	}
	if err := validatePolicyID("aethercore off-chain renewal provider key", renewal.ProviderKey); err != nil {
		return err
	}
	if renewal.RenewedAtHeight == 0 {
		return errors.New("aethercore off-chain renewal height must be positive")
	}
	if renewal.ExpiryHeight <= renewal.RenewedAtHeight {
		return errors.New("aethercore off-chain renewal expiry must exceed renewal height")
	}
	if renewal.Signer != renewal.ProviderKey {
		return errors.New("aethercore off-chain endpoint renewal must be signed by provider")
	}
	if err := ValidateHash("aethercore off-chain renewal advertisement hash", renewal.AdvertisementHash); err != nil {
		return err
	}
	if expected := ComputeOffChainEndpointRenewalHash(renewal); renewal.AdvertisementHash != expected {
		return fmt.Errorf("aethercore off-chain renewal hash mismatch: expected %s", expected)
	}
	if err := ValidateHash("aethercore off-chain renewal signature", renewal.SignatureHash); err != nil {
		return err
	}
	if expected := ComputeOffChainEndpointRenewalSignatureHash(renewal, renewal.Signer); renewal.SignatureHash != expected {
		return fmt.Errorf("aethercore off-chain renewal signature mismatch: expected %s", expected)
	}
	return nil
}
...
func ComputeOffChainServiceDescriptorSchemaHash(schema OffChainServiceDescriptorSchema) string {
	return hashParts(
		"aetheris-aek-offchain-service-schema-v1",
...
func ComputeOffChainAdvertisementHash(ad OffChainSignedAdvertisement) string {
	return hashParts(
		"aetheris-aek-offchain-advertisement-v1",
		ad.ServiceID,
		ad.Owner,
		ad.InterfaceHash,
		ad.Endpoint,
...
func ComputeOffChainAdvertisementSignatureHash(ad OffChainSignedAdvertisement, signer string) string {
	return hashParts("aetheris-aek-offchain-advertisement-signature-v1", ad.AdvertisementHash, signer)
}

func ComputeOffChainServiceRequestHash(request OffChainServiceRequest) string {
	return hashParts(
		"aetheris-aek-offchain-request-v1",
		request.CallID,
		request.ServiceID,
		request.MethodID,
		request.Caller,
...
func ComputeOffChainRequestSignatureHash(request OffChainServiceRequest, signer string) string {
	return hashParts("aetheris-aek-offchain-request-signature-v1", request.RequestHash, signer)
}

func ComputeOffChainResponseSignatureHash(response OffChainServiceResponse, signer string) string {
	return hashParts(
		"aetheris-aek-offchain-response-signature-v1",
		response.CallID,
		response.ServiceID,
		response.MethodID,
...
func ComputeOffChainReceiptAnchorProofHash(anchor OffChainReceiptAnchorMessage) string {
	return hashParts(
		"aetheris-aek-offchain-receipt-proof-anchor-v1",
		anchor.RequestHash,
		anchor.ResponseHash,
		anchor.ProviderKey,
...
func ComputeOffChainReceiptAnchorHash(anchor OffChainReceiptAnchorMessage) string {
	return hashParts(
		"aetheris-aek-offchain-receipt-anchor-v1",
		anchor.ServiceID,
		anchor.CallID,
		anchor.RequestHash,
		anchor.ResponseHash,
...
func ComputeOffChainEndpointRenewalHash(renewal OffChainEndpointRenewal) string {
	return hashParts(
		"aetheris-aek-offchain-endpoint-renewal-v1",
		renewal.ServiceID,
		renewal.Endpoint,
...
func ComputeOffChainEndpointRenewalSignatureHash(renewal OffChainEndpointRenewal, signer string) string {
	return hashParts("aetheris-aek-offchain-endpoint-renewal-signature-v1", renewal.AdvertisementHash, signer)
}
...
func validateOffChainMethods(methods []OffChainServiceMethod) error {
	if len(methods) == 0 {
		return errors.New("aethercore off-chain service requires method set")
	}
	var previous string
	...
	for i, method := range methods {
		...
		if _, found := seen[method.MethodID]; found {
			return fmt.Errorf("duplicate aethercore off-chain method %s", method.MethodID)
		}
		seen[method.MethodID] = struct{}{}
		if i > 0 && previous >= method.MethodID {
			return errors.New("aethercore off-chain methods must be sorted canonically")
		}
