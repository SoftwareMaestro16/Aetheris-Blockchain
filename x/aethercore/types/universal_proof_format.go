...
func (h UniversalTrustedHeader) Validate() error {
	if h.ChainID == "" {
		return errors.New("aethercore universal proof trusted header chain_id is required")
	}
	if h.Height == 0 {
		return errors.New("aethercore universal proof trusted header height must be positive")
	}
	if err := ValidateHash("aethercore universal proof trusted header app hash", h.AppHash); err != nil {
		return err
	}
	if h.HeaderHash != "" {
		return ValidateHash("aethercore universal proof trusted header hash", h.HeaderHash)
	}
	return nil
}
...
func (p UniversalStoreProof) Validate() error {
	if p.ProofVersion != UniversalProofVersionV1 {
		return fmt.Errorf("aethercore universal store proof version %d is not supported", p.ProofVersion)
	}
	if len(p.Key) == 0 {
		return errors.New("aethercore universal store proof key is required")
	}
	if err := ValidateHash("aethercore universal store proof root", p.StoreRoot); err != nil {
		return err
	}
	for _, op := range p.ProofOps {
		if err := ValidateHash("aethercore universal store proof op", op); err != nil {
			return err
		}
	}
	if err := ValidateHash("aethercore universal store proof hash", p.ProofHash); err != nil {
		return err
	}
	if p.ProofHash != ComputeUniversalStoreProofHash(p) {
		return errors.New("aethercore universal store proof hash mismatch")
	}
	return nil
}

func (s UniversalRootStep) Validate() error {
	if err := ValidateHash("aethercore universal proof path parent root", s.FromRoot); err != nil {
		return err
	}
	if err := ValidateHash("aethercore universal proof path child root", s.ToRoot); err != nil {
		return err
	}
	if err := validateToken("aethercore universal proof path parent root type", string(s.FromRootType), MaxScopeLength); err != nil {
		return err
	}
	if err := validateToken("aethercore universal proof path child root type", string(s.ToRootType), MaxScopeLength); err != nil {
		return err
	}
	if s.Scope != "" {
		if err := validateToken("aethercore universal proof path scope", s.Scope, MaxScopeLength); err != nil {
			return err
		}
	}
	if err := ValidateHash("aethercore universal proof path step hash", s.StepHash); err != nil {
		return err
	}
	if s.StepHash != ComputeUniversalRootStepHash(s) {
		return errors.New("aethercore universal proof path step hash mismatch")
	}
	return nil
}
...
func (c UniversalShardCommitment) Validate() error {
	if c.Height == 0 {
		return errors.New("aethercore universal shard commitment height must be positive")
	}
	...
	if err := ValidateHash("aethercore universal shard commitment shard root", c.ShardRoot); err != nil {
		return err
	}
	if err := ValidateHash("aethercore universal shard commitment aggregate root", c.ShardRootsRoot); err != nil {
		return err
	}
	if err := ValidateHash("aethercore universal shard commitment hash", c.CommitmentHash); err != nil {
		return err
	}
	if c.CommitmentHash != ComputeUniversalShardCommitmentHash(c) {
		return errors.New("aethercore universal shard commitment hash mismatch")
	}
	return nil
}
...
func (c UniversalMessageCommitment) Validate() error {
	if c.Height == 0 {
		return errors.New("aethercore universal message commitment height must be positive")
	}
	for _, field := range []struct {
		name  string
		value string
	}{
		{"aethercore universal message commitment message id", c.MessageID},
		{"aethercore universal message root", c.MessageRoot},
		{"aethercore universal source outbox root", c.SourceOutboxRoot},
		{"aethercore universal destination inbox root", c.DestinationInboxRoot},
		{"aethercore universal receipt root", c.ReceiptRoot},
		{"aethercore universal receipt hash", c.ReceiptHash},
		{"aethercore universal message commitment hash", c.MessageCommitmentHash},
		{"aethercore universal receipt commitment hash", c.ReceiptCommitmentHash},
		{"aethercore universal delivery commitment hash", c.DeliveryCommitmentHash},
	} {
	...
	if c.MessageCommitmentHash != ComputeUniversalMessageCommitmentHash(c) {
		return errors.New("aethercore universal message commitment hash mismatch")
	}
	if c.ReceiptCommitmentHash != ComputeUniversalReceiptCommitmentHash(c) {
		return errors.New("aethercore universal receipt commitment hash mismatch")
	}
	if c.DeliveryCommitmentHash != ComputeUniversalDeliveryCommitmentHash(c) {
		return errors.New("aethercore universal delivery commitment hash mismatch")
	}
	return nil
}
...
func (p UniversalProofEnvelope) ValidateFormat() error {
	if p.ProofVersion != UniversalProofVersionV1 {
		return fmt.Errorf("aethercore universal proof version %d is not supported", p.ProofVersion)
	}
	if p.ChainID == "" {
		return errors.New("aethercore universal proof chain_id is required")
	}
	if p.Height == 0 {
		return errors.New("aethercore universal proof height must be positive")
	}
	if err := ValidateHash("aethercore universal proof app hash", p.AppHash); err != nil {
		return err
	}
	if err := validateToken("aethercore universal proof type", string(p.ProofType), MaxScopeLength); err != nil {
		return err
	}
	if err := validateToken("aethercore universal proof root type", string(p.RootType), MaxScopeLength); err != nil {
		return err
	}
	if len(p.Key) == 0 {
		return errors.New("aethercore universal proof key is required")
	}
	if len(p.VerificationPath) == 0 {
		return errors.New("aethercore universal proof verification path is required")
	}
	if p.ZoneID != "" {
	...
	if err := ValidateHash("aethercore universal proof envelope hash", p.ProofHash); err != nil {
		return err
	}
	if p.ProofHash != ComputeUniversalProofEnvelopeHash(p) {
		return errors.New("aethercore universal proof envelope hash mismatch")
	}
	return nil
}
...
func (p UniversalProofEnvelope) validateCompatibility() error {
	...
	if !found {
		return fmt.Errorf("aethercore universal proof type %q is not supported", p.ProofType)
	}
	if req.RootType != "" && p.RootType != req.RootType {
		return fmt.Errorf("aethercore proof type %s requires root type %s", p.ProofType, req.RootType)
	}
	if req.ZoneID != "" && p.ZoneID != req.ZoneID {
		return fmt.Errorf("aethercore proof type %s requires zone %s", p.ProofType, req.ZoneID)
	}
	if req.RequiresZone && (!p.HasZoneCommit || p.ZoneID == "") {
		return fmt.Errorf("aethercore proof type %s requires zone commitment", p.ProofType)
	}
	if req.RequiresShard && (!p.HasShardCommit || p.ShardID == "") {
		return fmt.Errorf("aethercore proof type %s requires shard commitment", p.ProofType)
	}
	if req.RequiresMessage && !p.HasMessageCommit {
		return fmt.Errorf("aethercore proof type %s requires message commitment", p.ProofType)
	}
	if req.RequiresAbsence && len(p.AbsenceMarker) == 0 {
		return fmt.Errorf("aethercore proof type %s requires non-existence marker", p.ProofType)
	}
	return nil
}

func ComputeUniversalStoreProofHash(proof UniversalStoreProof) string {
	return hashRoot("aetheris-next-universal-store-proof-v1", func(w byteWriter) {
...
func ComputeUniversalRootStepHash(step UniversalRootStep) string {
	return hashRoot("aetheris-next-universal-root-step-v1", func(w byteWriter) {
...
func ComputeUniversalShardCommitmentHash(commitment UniversalShardCommitment) string {
	return hashRoot("aetheris-next-universal-shard-commitment-v1", func(w byteWriter) {
...
func ComputeUniversalMessageCommitmentHash(commitment UniversalMessageCommitment) string {
	return hashRoot("aetheris-next-universal-message-commitment-v1", func(w byteWriter) {
		writeUint64(w, commitment.Height)
...
func ComputeUniversalReceiptCommitmentHash(commitment UniversalMessageCommitment) string {
	return hashRoot("aetheris-next-universal-receipt-commitment-v1", func(w byteWriter) {
		writeUint64(w, commitment.Height)
		writePart(w, commitment.MessageID)
...
func ComputeUniversalDeliveryCommitmentHash(commitment UniversalMessageCommitment) string {
	return hashRoot("aetheris-next-universal-delivery-commitment-v1", func(w byteWriter) {
		writeUint64(w, commitment.Height)
		writePart(w, commitment.MessageID)
...
func ComputeUniversalProofEnvelopeHash(proof UniversalProofEnvelope) string {
	return hashRoot("aetheris-next-universal-proof-envelope-v1", func(w byteWriter) {
...
func verifyUniversalRootPath(proof UniversalProofEnvelope) error {
	...
	for i, step := range proof.VerificationPath {
		if uint32(i) != step.Index {
			return errors.New("aethercore universal proof path index mismatch")
		}
		if step.FromRoot != current {
			return errors.New("aethercore universal proof path is not contiguous")
		}
		if err := step.Validate(); err != nil {
			return err
		}
	...
	if current != proof.StoreProof.StoreRoot {
		return errors.New("aethercore universal proof path does not end at store root")
	}
	last := proof.VerificationPath[len(proof.VerificationPath)-1]
	if last.ToRootType != proof.RootType {
		return fmt.Errorf("aethercore universal proof path ends at root type %s, expected %s", last.ToRootType, proof.RootType)
	}
	return nil
}
...
func verifyUniversalZoneScope(proof UniversalProofEnvelope) error {
	...
	if !proof.HasZoneCommit {
		return errors.New("aethercore universal proof missing zone commitment")
	}
	if proof.ZoneCommitment.Height != proof.Height {
		return errors.New("aethercore universal proof zone commitment height mismatch")
	}
	if proof.ZoneCommitment.ZoneID != proof.ZoneID {
		return errors.New("aethercore universal proof zone commitment zone mismatch")
	}
	if proof.ProofType == ProofTypeZoneRoot && proof.ZoneCommitment.StateRoot != proof.StoreProof.StoreRoot {
		return errors.New("aethercore universal proof zone state root mismatch")
	}
	return nil
}
...
func verifyUniversalShardScope(proof UniversalProofEnvelope) error {
	...
	if !proof.HasShardCommit {
		return errors.New("aethercore universal proof missing shard commitment")
	}
	if proof.ShardCommitment.Height != proof.Height {
		return errors.New("aethercore universal proof shard commitment height mismatch")
	}
	if proof.ShardCommitment.ZoneID != proof.ZoneID || proof.ShardCommitment.ShardID != proof.ShardID {
		return errors.New("aethercore universal proof shard commitment scope mismatch")
	}
	if proof.HasZoneCommit && proof.ShardCommitment.ShardRootsRoot != proof.ZoneCommitment.ShardRootsRoot {
		return errors.New("aethercore universal proof shard aggregate root mismatch")
	}
	if proof.ProofType == ProofTypeShardRoot && proof.ShardCommitment.ShardRoot != proof.StoreProof.StoreRoot {
		return errors.New("aethercore universal proof shard state root mismatch")
	}
	return nil
}
...
func verifyUniversalStoreProof(proof UniversalProofEnvelope) error {
	if string(proof.Key) != string(proof.StoreProof.Key) {
		return errors.New("aethercore universal proof store key mismatch")
	}
	if proof.ProofType == ProofTypeNonExistence {
		if len(proof.StoreProof.NonExistenceMarker) == 0 || len(proof.AbsenceMarker) == 0 {
			return errors.New("aethercore universal non-existence marker is required")
		}
		if string(proof.StoreProof.NonExistenceMarker) != string(proof.AbsenceMarker) {
			return errors.New("aethercore universal non-existence marker mismatch")
		}
		if len(proof.StoreProof.Value) != 0 || len(proof.Value) != 0 {
			return errors.New("aethercore universal non-existence proof must not carry value")
		}
		return nil
	}
	if len(proof.Value) == 0 || len(proof.StoreProof.Value) == 0 {
		return errors.New("aethercore universal existence proof value is required")
	}
	if string(proof.Value) != string(proof.StoreProof.Value) {
		return errors.New("aethercore universal proof value mismatch")
	}
	return nil
}
...
func verifyUniversalMessageScope(proof UniversalProofEnvelope) error {
	...
	if proof.MessageCommit.Height != proof.Height {
		return errors.New("aethercore universal proof message commitment height mismatch")
	}
	switch proof.ProofType {
	case ProofTypeMessageInclusion:
		if proof.MessageCommit.MessageRoot != proof.StoreProof.StoreRoot {
			return errors.New("aethercore universal proof message root mismatch")
		}
	case ProofTypeMessageReceipt:
		if proof.MessageCommit.ReceiptRoot != proof.StoreProof.StoreRoot {
			return errors.New("aethercore universal proof receipt root mismatch")
		}
		if proof.MessageCommit.ReceiptHash == EmptyRootHash {
			return errors.New("aethercore universal proof receipt not found")
		}
	}
	return nil
}
