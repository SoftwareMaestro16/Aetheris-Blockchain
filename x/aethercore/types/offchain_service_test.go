...
func TestOffChainEndpointRenewalExtendsExpiryAndRequiresProviderSignature(t *testing.T) {
	...
	renewal, err := NewOffChainEndpointRenewal(schema, "https://indexer2.aetheris.local/v1", 120, schema.Endpoint.ProviderKey)
	require.NoError(t, err)
	...
	_, err = NewOffChainEndpointRenewal(schema, "https://indexer3.aetheris.local/v1", schema.Endpoint.ExpiryHeight, schema.Endpoint.ProviderKey)
