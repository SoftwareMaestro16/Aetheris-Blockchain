...
func ServiceDescriptorStateKey(serviceID string) (string, error) {
	if err := validatePolicyID("aethercore service registry state service id", serviceID); err != nil {
		return "", err
	}
...
func ServiceAnchorStateKey(serviceID string) (string, error) {
	if err := validatePolicyID("aethercore service registry anchor service id", serviceID); err != nil {
		return "", err
	}
...
func ServiceInterfaceStateKey(interfaceHash string) (string, error) {
	if err := ValidateHash("aethercore service registry interface hash", interfaceHash); err != nil {
		return "", err
	}
...
func ServiceOwnerStateKey(owner, serviceID string) (string, error) {
	if err := addressing.ValidateAuthorityAddress("aethercore service registry state owner", owner); err != nil {
		return "", err
	}
	if err := validatePolicyID("aethercore service registry owner service id", serviceID); err != nil {
		return "", err
	}
...
func ServiceNameStateKey(serviceName string) (string, error) {
	if err := validatePolicyID("aethercore service registry service name", serviceName); err != nil {
		return "", err
	}
...
func IdentityServiceBindingStateKey(identityName, serviceID string) (string, error) {
	if err := validatePolicyID("aethercore service registry identity name", identityName); err != nil {
		return "", err
	}
	if err := validatePolicyID("aethercore service registry identity service id", serviceID); err != nil {
		return "", err
	}
...
func ServiceProviderStateKey(serviceID, providerID string) (string, error) {
	if err := validatePolicyID("aethercore service registry provider service id", serviceID); err != nil {
		return "", err
	}
	if err := validatePolicyID("aethercore service registry provider id", providerID); err != nil {
		return "", err
	}
...
func ServiceExpiryStateKey(expiryHeight uint64, serviceID string) (string, error) {
	if expiryHeight == 0 {
		return "", errors.New("aethercore service registry expiry height must be positive")
	}
	if err := validatePolicyID("aethercore service registry expiry service id", serviceID); err != nil {
		return "", err
	}
...
func ServiceReputationStateKey(providerID string) (string, error) {
	if err := validatePolicyID("aethercore service registry reputation provider id", providerID); err != nil {
		return "", err
	}
...
func ServiceReceiptStateKey(serviceID, callID string) (string, error) {
	if err := validatePolicyID("aethercore service registry receipt service id", serviceID); err != nil {
		return "", err
	}
	if err := ValidateHash("aethercore service registry receipt call id", callID); err != nil {
		return "", err
	}
...
func NewIdentityServiceBindingFromDescriptor(descriptor ServiceDescriptor) (IdentityServiceBinding, error) {
	...
	if descriptor.Discovery.IdentityName == "" {
		return IdentityServiceBinding{}, errors.New("aethercore identity service binding requires identity name")
	}
...
func NewServiceRegistryState(descriptors []ServiceDescriptor, anchors []ServiceAnchor, identityBindings []IdentityServiceBinding, providers []ProviderRecord, reputations []ReputationRecord, receipts []ServiceReceipt, height uint64) (ServiceRegistryState, error) {
	if height == 0 {
		return ServiceRegistryState{}, errors.New("aethercore service registry state height must be positive")
	}
...
func (anchor ServiceAnchor) Validate() error {
	anchor = CanonicalServiceAnchor(anchor)
	if err := validatePolicyID("aethercore service anchor service id", anchor.ServiceID); err != nil {
		return err
	}
	if err := ValidateHash("aethercore service anchor descriptor hash", anchor.DescriptorHash); err != nil {
		return err
	}
	if err := ValidateHash("aethercore service anchor interface hash", anchor.InterfaceHash); err != nil {
		return err
	}
	if anchor.ProviderRoot != "" {
		if err := ValidateHash("aethercore service anchor provider root", anchor.ProviderRoot); err != nil {
			return err
		}
	}
	if !IsServiceVerificationModel(anchor.VerificationModel) {
		return fmt.Errorf("unknown aethercore service anchor verification model %q", anchor.VerificationModel)
	}
	if err := ValidateHash("aethercore service anchor hash", anchor.AnchorHash); err != nil {
		return err
	}
	if expected := ComputeServiceAnchorHash(anchor); anchor.AnchorHash != expected {
		return fmt.Errorf("aethercore service anchor hash mismatch: expected %s", expected)
	}
	return nil
}
...
func (binding IdentityServiceBinding) Validate() error {
	binding = CanonicalIdentityServiceBinding(binding)
	if err := validatePolicyID("aethercore identity service binding identity name", binding.IdentityName); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore identity service binding service id", binding.ServiceID); err != nil {
		return err
	}
	if err := addressing.ValidateAuthorityAddress("aethercore identity service binding owner", binding.Owner); err != nil {
		return err
	}
	if err := ValidateHash("aethercore identity service binding descriptor hash", binding.DescriptorHash); err != nil {
		return err
	}
	if binding.CreatedHeight == 0 {
		return errors.New("aethercore identity service binding created height must be positive")
	}
	if binding.ExpiryHeight != 0 && binding.ExpiryHeight <= binding.CreatedHeight {
		return errors.New("aethercore identity service binding expiry must exceed created height")
	}
	if err := ValidateHash("aethercore identity service binding hash", binding.BindingHash); err != nil {
		return err
	}
	if expected := ComputeIdentityServiceBindingHash(binding); binding.BindingHash != expected {
		return fmt.Errorf("aethercore identity service binding hash mismatch: expected %s", expected)
	}
	return nil
}
...
func (record ProviderRecord) Validate() error {
	...
	if err := validatePolicyID("aethercore service provider record service id", record.ServiceID); err != nil {
		return err
	}
	if err := record.Provider.Validate(); err != nil {
		return err
	}
	if err := ValidateHash("aethercore service provider record hash", record.RecordHash); err != nil {
		return err
	}
	if expected := ComputeProviderRecordHash(record); record.RecordHash != expected {
		return fmt.Errorf("aethercore service provider record hash mismatch: expected %s", expected)
	}
	return nil
}
...
func (record ReputationRecord) Validate() error {
	...
	if err := validatePolicyID("aethercore reputation provider id", record.ProviderID); err != nil {
		return err
	}
	if record.UpdatedHeight == 0 {
		return errors.New("aethercore reputation record updated height must be positive")
	}
	if err := ValidateHash("aethercore reputation record hash", record.RecordHash); err != nil {
		return err
	}
	if expected := ComputeReputationRecordHash(record); record.RecordHash != expected {
		return fmt.Errorf("aethercore reputation record hash mismatch: expected %s", expected)
	}
	return nil
}
...
func (entry ServiceRegistryStateEntry) Validate() error {
	if !IsServiceRegistryStateEntryType(entry.EntryType) {
		return fmt.Errorf("unknown aethercore service registry state entry type %q", entry.EntryType)
	}
	if err := validateServiceRegistryStateKey(entry.Key); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore service registry state entry value", entry.Value); err != nil {
		return err
	}
	if err := ValidateHash("aethercore service registry state entry hash", entry.EntryHash); err != nil {
		return err
	}
	if expected := ComputeServiceRegistryStateEntryHash(entry); entry.EntryHash != expected {
		return fmt.Errorf("aethercore service registry state entry hash mismatch: expected %s", expected)
	}
	return nil
}
...
func (state ServiceRegistryState) Validate() error {
	if state.UpdatedHeight == 0 {
		return errors.New("aethercore service registry state height must be positive")
	}
	...
	if err := ValidateHash("aethercore service registry state root", state.StateRoot); err != nil {
		return err
	}
	if expected := ComputeServiceRegistryStateRoot(state); state.StateRoot != expected {
		return fmt.Errorf("aethercore service registry state root mismatch: expected %s", expected)
	}
	return nil
}
...
func ComputeServiceAnchorHash(anchor ServiceAnchor) string {
	anchor.AnchorHash = ""
	return hashParts(
		"aetheris-aek-service-anchor-v1",
		anchor.ServiceID,
...
func ComputeIdentityServiceBindingHash(binding IdentityServiceBinding) string {
	binding.BindingHash = ""
	return hashParts(
		"aetheris-aek-identity-service-binding-v1",
...
func ComputeProviderRecordHash(record ProviderRecord) string {
	record.RecordHash = ""
	return hashParts(
		"aetheris-aek-service-provider-record-v1",
		record.ServiceID,
...
func ComputeReputationRecordHash(record ReputationRecord) string {
	record.RecordHash = ""
	return hashParts(
		"aetheris-aek-reputation-record-v1",
		record.ProviderID,
...
func ComputeServiceRegistryStateEntryHash(entry ServiceRegistryStateEntry) string {
	entry.EntryHash = ""
	return hashParts(
		"aetheris-aek-service-registry-state-entry-v1",
...
func ComputeServiceRegistryStateRoot(state ServiceRegistryState) string {
	...
	parts := []string{
		"aetheris-aek-service-registry-state-root-v1",
...
func addServiceRegistryStateEntry(entries map[string]ServiceRegistryStateEntry, entryType ServiceRegistryStateEntryType, key, value string) error {
	...
	if found {
		if existing.Value != entry.Value || existing.EntryType != entry.EntryType {
			return fmt.Errorf("aethercore service registry state key collision %s", key)
		}
		return nil
	}
	entries[key] = entry
...
func validateServiceRegistryStateKey(key string) error {
	if err := validatePolicyID("aethercore service registry state key", key); err != nil {
		return err
	}
	if !strings.HasPrefix(key, "services/") {
		return errors.New("aethercore service registry state key must use services prefix")
	}
	if strings.Contains(key, "//") {
		return errors.New("aethercore service registry state key contains empty segment")
	}
	return nil
}
...
func validateServiceRegistryStateEntries(entries []ServiceRegistryStateEntry) error {
	...
	for _, entry := range entries {
		...
		if _, found := seen[entry.Key]; found {
			return fmt.Errorf("duplicate aethercore service registry state key %s", entry.Key)
		}
		seen[entry.Key] = struct{}{}
		if previous != "" && previous >= entry.Key {
			return errors.New("aethercore service registry state entries must be sorted canonically")
		}
		previous = entry.Key
	}
	return nil
