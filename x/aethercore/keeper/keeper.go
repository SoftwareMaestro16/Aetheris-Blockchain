...
type GenesisState struct {
	Version uint64
	Params  types.AetherCoreParams
	State   types.AetherCoreState
}
...
func (gs GenesisState) Validate() error {
	if gs.Version != prototype.CurrentGenesisVersion {
		return errors.New("aethercore prototype unsupported genesis version")
	}
...
func (k *Keeper) UpdateParams(authority string, params types.AetherCoreParams) error {
...
func normalizePage(req *prototype.PageRequest, params types.AetherCoreParams, total int) (start int, end int, res prototype.PageResponse, err error) {
	if err := params.Validate(); err != nil {
	...
	if limit == 0 || limit > params.MaxQueryLimit {
		return 0, 0, prototype.PageResponse{}, errors.New("aethercore query limit out of bounds")
	}
	if req.Offset > uint64(total) {
		return 0, 0, prototype.PageResponse{}, errors.New("aethercore query offset out of bounds")
	}
	start = int(req.Offset)
	end64 := req.Offset + limit
	if end64 < req.Offset {
		return 0, 0, prototype.PageResponse{}, errors.New("aethercore query offset overflow")
	}
