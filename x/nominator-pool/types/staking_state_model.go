...
func (a StakeReputationAccumulator) Validate() error {
	if err := ValidateUserFacingAEAddress("stake reputation account", a.Account); err != nil {
		return err
	}
	if a.LastUpdatedHeight == 0 {
		return errors.New("stake reputation accumulator height must be positive")
	}
	return nil
}
