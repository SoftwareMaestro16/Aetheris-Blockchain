...
import (
	...
	"github.com/sovereign-l1/l1/x/mint-authority/keeper"
...
func (AppModule) RegisterInterfaces(registry codectypes.InterfaceRegistry) {
	mintauthoritypb.RegisterInterfaces(registry)
}

func (AppModule) RegisterGRPCGatewayRoutes(clientCtx client.Context, mux *runtime.ServeMux) {
	if err := mintauthoritypb.RegisterQueryHandlerClient(context.Background(), mux, mintauthoritypb.NewQueryClient(clientCtx)); err != nil {
		panic(err)
	}
}

func (am AppModule) RegisterServices(cfg module.Configurator) {
	mintauthoritypb.RegisterMsgServer(cfg.MsgServer(), keeper.NewMsgServerImpl(am.keeper))
	mintauthoritypb.RegisterQueryServer(cfg.QueryServer(), am.keeper)
}
...
func (am AppModule) ValidateGenesis(cdc codec.JSONCodec, _ client.TxEncodingConfig, bz json.RawMessage) error {
	var gs types.MintAuthorityState
	if err := json.Unmarshal(bz, &gs); err != nil {
		return fmt.Errorf("failed to unmarshal %s genesis state: %w", types.ModuleName, err)
	}
	return types.NormalizeMintAuthorityState(gs).Validate()
}
