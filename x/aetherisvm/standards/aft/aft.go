...
const (
	...
	walletDerivationDomain   = "aetheris/AFT-44/wallet/v1"
)
...
func DeriveWalletAddress(master, owner sdk.AccAddress, walletCodeHash []byte) (sdk.AccAddress, error) {
	if err := aetherisaddress.RejectZeroAddress("token master", master); err != nil {
		return nil, err
	}
	if err := aetherisaddress.RejectZeroAddress("wallet owner", owner); err != nil {
		return nil, err
	}
...
func (m MasterState) Validate() error {
	if err := aetherisaddress.RejectZeroAddress("token master", m.Address); err != nil {
		return err
	}
	if !m.AdminRenounced {
		if err := aetherisaddress.RejectZeroAddress("token admin", m.Admin); err != nil {
			return err
		}
	}
...
func (w WalletState) Validate() error {
	if err := aetherisaddress.RejectZeroAddress("token wallet", w.Address); err != nil {
		return err
	}
	if err := aetherisaddress.RejectZeroAddress("token master", w.MasterAddress); err != nil {
		return err
	}
...
func (s *State) ChangeAdmin(caller, nextAdmin sdk.AccAddress) error {
	...
	if err := aetherisaddress.RejectZeroAddress("new token admin", nextAdmin); err != nil {
		return err
	}
