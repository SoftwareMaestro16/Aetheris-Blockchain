...
func NewCreatePoolCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			msg := &types.MsgCreatePool{Creator: aetherisaddress.FormatAccAddress(clientCtx.GetFromAddress()), TokenA: a, TokenB: b}
...
func NewAddLiquidityCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			msg := &types.MsgAddLiquidity{Depositor: aetherisaddress.FormatAccAddress(clientCtx.GetFromAddress()), PoolId: poolID, TokenA: a, TokenB: b, MinShares: args[3]}
			return tx.GenerateOrBroadcastTxCLI(clientCtx, cmd.Flags(), msg)
		},
	}
	flags.AddTxFlagsToCmd(cmd)
...
func NewRemoveLiquidityCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			msg := &types.MsgRemoveLiquidity{Withdrawer: aetherisaddress.FormatAccAddress(clientCtx.GetFromAddress()), PoolId: poolID, Shares: shares}
			return tx.GenerateOrBroadcastTxCLI(clientCtx, cmd.Flags(), msg)
		},
	}
	flags.AddTxFlagsToCmd(cmd)
...
func NewSwapCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			msg := &types.MsgSwapExactAmountIn{Trader: aetherisaddress.FormatAccAddress(clientCtx.GetFromAddress()), PoolId: poolID, TokenIn: in, TokenOutDenom: args[2], MinAmountOut: args[3]}
			return tx.GenerateOrBroadcastTxCLI(clientCtx, cmd.Flags(), msg)
		},
	}
	flags.AddTxFlagsToCmd(cmd)
