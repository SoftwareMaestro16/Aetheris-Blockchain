...
import (
	...
	"github.com/sovereign-l1/l1/x/performance/keeper"
...
func (AppModule) RegisterInterfaces(registry codectypes.InterfaceRegistry) {
	performancepb.RegisterInterfaces(registry)
}

func (AppModule) RegisterGRPCGatewayRoutes(clientCtx client.Context, mux *runtime.ServeMux) {
	if err := performancepb.RegisterQueryHandlerClient(context.Background(), mux, performancepb.NewQueryClient(clientCtx)); err != nil {
		panic(err)
	}
}

func (am AppModule) RegisterServices(cfg module.Configurator) {
	performancepb.RegisterMsgServer(cfg.MsgServer(), keeper.NewMsgServerImpl(am.keeper))
	performancepb.RegisterQueryServer(cfg.QueryServer(), am.keeper)
}
...
func (am AppModule) ValidateGenesis(cdc codec.JSONCodec, _ client.TxEncodingConfig, bz json.RawMessage) error {
	var gs types.PerformanceOracleState
	if err := json.Unmarshal(bz, &gs); err != nil {
		return fmt.Errorf("failed to unmarshal %s genesis state: %w", types.ModuleName, err)
	}
	return types.NormalizePerformanceOracleState(gs).Validate()
}
